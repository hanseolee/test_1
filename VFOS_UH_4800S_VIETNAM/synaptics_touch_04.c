...
void spi_func_for_dp173(u8* w_buf, u8* r_buf, u16 buf_len)
{
	int i, arg;
...
// not used
void init_tch_power_set_for_dp173_synap(int on)
{
	int power = 0;
...
static struct synaptics_04_touch_limit l_limit;		// local limit : use this

int synaptics_04_touch_limit_table_parser(MODEL id, char *m_name, struct synaptics_04_touch_limit* limit)
{
	char string[500];
	FILE *fp;
	char *token = NULL;
		...
		while(token != NULL){
			...
			else
			{
				...
				{	
					...
					case 1:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->fw_ver))
							mode = 0; 
						break;
					case 2:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->config_ver))
							mode = 0; 
						break;
					case 3:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->product_id))
							mode = 0; 
						break;
					case 4:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->full_raw_cap_MIN))
							mode = 0; 
						break;
					case 5:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->full_raw_cap_MAX))
							mode = 0; 
						break;
					case 6:
						if(synaptics_04_touch_csv_parser3(&x, &y, &row, token, limit->extend_high_regi_MIN))
							mode = 0; 
						break;
					case 7:
						if(synaptics_04_touch_csv_parser3(&x, &y, &row, token, limit->extend_high_regi_MAX))
							mode = 0; 
						break;
					case 8:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->hybrid_raw_cap_TX_MIN))
							mode = 0; 
						break;
					case 9:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->hybrid_raw_cap_TX_MAX))
							mode = 0; 
						break;
					case 10:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->hybrid_raw_cap_RX_MIN))
							mode = 0; 
						break;
					case 11:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->hybrid_raw_cap_RX_MAX))
							mode = 0; 
						break;
					case 12:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->customer_id))
							mode = 0; 
						break;
					case 13:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->cm_jitter_MIN))
							mode = 0; 
						break;
					case 14:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->cm_jitter_MAX))
							mode = 0; 
						break;
					case 15:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->sensor_MIN))
							mode = 0; 
						break;
					case 16:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->sensor_MAX))
							mode = 0; 
						break;
					case 17:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->avdd_MAX))
							mode = 0; 
						break;
					case 18:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->dvdd_MAX))
							mode = 0; 
						break;
					case 19:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->ADC_RANGE_MIN))
							mode = 0; 
						break;
					case 20:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->ADC_RANGE_MAX))
							mode = 0; 
						break;
					case 21:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->full_raw_cap_h))
							mode = 0; 
						break;
					case 22:
						if(synaptics_04_touch_csv_parser(&x, &y, &row, token, limit->full_raw_cap_v))
							mode = 0; 
						break;
					case 23:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->avdd_MIN))
							mode = 0;
						break;
					case 24:
						if(synaptics_04_touch_csv_parser2(&x, &y, &row, token, limit->dvdd_MIN))
							mode = 0;
						break;
				}	
			}
...
int synaptics_04_touch_csv_parser(int *xx, int *yy, int *row, char *token, long limit_array[][300])
{
	int i = 0, j =0;
	int n = 0;
	int flag = 0;
...
int synaptics_04_touch_csv_parser2(int *xx, int *yy, int *row, char *token, char *limit_array)
{
...
int synaptics_04_touch_csv_parser3(int *xx, int *yy, int *row, char *token, float limit_array[][300])
{
	int i = 0, j =0;
	int n = 0;
	int flag = 0;
...
void synaptics_04_init_limit_data(struct synaptics_04_touch_limit *limit){
	l_limit = *limit;
}

u8 synap_rbuf_for_dp173[200] = {0xff, };	// rbuf will duplicate somewhere.. static impossible, because functions are not static..
...
uint16_t fullraw_for_dp173[(IMAGE_ROW)*(IMAGE_COL)] = {0xffff, };					// TD06 FULL RAW CAP
int16_t fullraw_for_dp173_h_for_dp173[(IMAGE_ROW-1)*(IMAGE_COL)] = {0xffff, };				// TD19 FULL RAW H : 음수가능
int16_t fullraw_for_dp173_v_for_dp173[(IMAGE_ROW)*(IMAGE_COL-1)] = {0xffff, };				// TD20 FULL RAW V : 음수가능

uint16_t open_short_for_dp173[(IMAGE_ROW)*(IMAGE_COL)] = {0xffff, };				// TD18 HIGH RESIST, not 18 x 36, 705...
uint32_t hybrid_abs_with_cbc_for_dp173[HYBRID_ABS_WITH_CBC] = {0xffffffff, };		// TD11 Hybrid_abs_with_cbc

int16_t noise_for_dp173[NOISE] = {0xffff, };										// TD07 NOISE TEST : 음수가능
int16_t sensor_for_dp173[SENSOR] = {0xffff, };									// TD08 SENSOR SPEED : 음수가능

//uint16_t adcrange_for_dp173[ADCRANGE] = {0xffff, };								// TD15 ADC RANGE TEST
uint16_t adcrange_for_dp173[(IMAGE_ROW)*(IMAGE_COL)] = {0xffff, };
...
#if 0
short p_ref_frame_for_dp173[NUM_TX * NUM_RX] = {
...
int synaptics_04_current_avdd_dvdd_check(int ch_num){
	int16_t avdd_cur1;
	int16_t dvdd_cur1;
	int16_t avdd_cur2;
	int16_t dvdd_cur2;
	...
	switch(ch_num){
		case 1:
			get_current_test_result_for_dp173_touch(&dvdd_cur1, &avdd_cur1, ch_num);
			...
			break;
		case 2:
			get_current_test_result_for_dp173_touch(&dvdd_cur2, &avdd_cur2, ch_num);
...
int synaptics_04_attention_check(){
	int res;									// PASS/FAIL	
	send_command_for_dp173(IDENTIFY);

	if((synap_rbuf_for_dp173[0] == 0xa5) && (synap_rbuf_for_dp173[1] == 0x10)){
		res = 1;		// PASS
	}else{
		res = 0;		// FAIL
	}
...
int synaptics_04_reset_pin_check(){
...
int synaptics_04_fw_version_check(){
	...
	// firmware build id
	build_id[0] = synap_rbuf_for_dp173[22];
	build_id[1] = synap_rbuf_for_dp173[23];
	build_id[2] = synap_rbuf_for_dp173[24];
	build_id[3] = synap_rbuf_for_dp173[25];
...
//#define CRC_OFFSET		8
int synaptics_04_touch_ic_config_check(){
	int res;									// SPEC : 0x29020000
	char config_id1[100];							

	send_command_for_dp173(GET_APP_INFO);

	// spec에 비해 rawdata 의 길이가 4배라서, 일단 4군데에 전부 받는다
	printf("CHECK CUSTOMER CONFIGURATION ID\n");		// start with 20, not 16
	{
		sprintf(config_id1, "%02X%02X%02X%02X%02X%02X%02X%02X", synap_rbuf_for_dp173[20], synap_rbuf_for_dp173[21], synap_rbuf_for_dp173[22], synap_rbuf_for_dp173[23], synap_rbuf_for_dp173[24], synap_rbuf_for_dp173[25], synap_rbuf_for_dp173[26], synap_rbuf_for_dp173[27]);
	}
...
int synaptics_04_device_package_check(){
	int res;									// SPEC : 3909
	u8 part_num[4];
	send_command_for_dp173(IDENTIFY);
	printf("CHECK PART NUMBER STRING\n");
	{
		printf("%s\n", synap_rbuf_for_dp173+6);	// "s3909-15.0.1\0"
	}

	part_num[0] = synap_rbuf_for_dp173[7];
	part_num[1] = synap_rbuf_for_dp173[8];
	part_num[2] = synap_rbuf_for_dp173[9];
	part_num[3] = synap_rbuf_for_dp173[10];
...
int synaptics_04_lockdown_check(){	
	...
	send_command_for_dp173_delay(ENTER_BOOTLOADER_MODE);
	send_command_for_dp173(GET_BOOT_INFO);	

	#if LOCKDOWN_DEBUG	
	printf("\n\n");
	printf("CHECK OTP BLOCK NUM / SIZE\n"); 	
	
	printf("OTP BLOCK NUM : 0x%02X%02X\n", synap_rbuf_for_dp173[17],synap_rbuf_for_dp173[16]);
	printf("OTP BLOCK SIZE : 0x%02X%02X\n", synap_rbuf_for_dp173[19],synap_rbuf_for_dp173[18]);
	#endif

	block_num = synap_rbuf_for_dp173[16] | synap_rbuf_for_dp173[17] << 8;
	block_size = synap_rbuf_for_dp173[18] | synap_rbuf_for_dp173[19] << 8;

	#if LOCKDOWN_DEBUG
	printf("WRITE BLOCK SIZE : 0x%02X\n", synap_rbuf_for_dp173[8]);
	#endif

	write_block_size = synap_rbuf_for_dp173[8];
	...
	send_command_for_dp173_multi_payload_output_buf_uint8_t(READ_FLASH, payload, sizeof(payload), otp_data);
	send_command_for_dp173_delay(RUN_APPLICATION_FIRMWARE);
...
int synaptics_04_attention2_check(int ch_num){	
#if 1
	...
	u8 payload[] = { FIELD_INT2, 3, 0 };		// 0 : FIELD ID
												// 1, 2 : FIELD VALUE
	send_command_for_dp173_multi_payload(SET_DYNAMIC_CONFIG, payload, sizeof(payload)); 
	usleep(30000);
...
#define OSC_DEBUG	0
int synaptics_04_osc_check(){
	int res;									// PASS/FAIL
	u8 payload[] = { TEST_EXTERNAL_OSC };
	send_command_for_dp173_multi_payload(PRODUCTION_TEST, payload, sizeof(payload));		// 05/01 NOT IMPLEMENTED!
	printf("osc value : 0x%x\n", synap_rbuf_for_dp173[1]);
	if(synap_rbuf_for_dp173[1] == 0x01){
		#if OSC_DEBUG
		printf("PASS\n");
		#endif
		res = 1;
...
int synaptics_04_bsc_calibration_check(){
	int res = 1;									// PASS/FAIL
	u8 payload[] = { 3 };		// ERASE COMMAND
	send_command_for_dp173_multi_payload(CALIBRATE, payload, sizeof(payload));
	if(synap_rbuf_for_dp173[1] == 0x01){
...
int synaptics_04_short_check(){	
	int res = 1;								// pass/fail
	int i, bit;
	uint8_t temp[8];
	uint8_t bitvalue, limit;
	int trx_sensor_for_dp173_result = 1;
	int trx_gnd_result = 1;

	// TRX7~0, 15~8, ... 63~56
	// TRX별 정해진 행, 또는 정해진 열을 체크(가령 TRX13번핀은 열을셀때 표에서 RX33채널에 위치, [13]열)
	u8 trx_sensor_for_dp173_spec[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	...
	send_command_for_dp173_multi_payload(PRODUCTION_TEST, payload, sizeof(payload));	

	// 담는다.
	for(i=0;i<8;i++){
		temp[i] = synap_rbuf_for_dp173[4+i];
	}

	// temp를 trx_sensor_for_dp173_spec과 비교한다.
	printf("TRX_SENSOR_OPEN TEST\n");
	for(i=0;i<8;i++){
		if(temp[i] != trx_sensor_for_dp173_spec[i]){
			printf("!");
			res = 0;
			trx_sensor_for_dp173_result = 0;
		}
	...
	// 결과를 출력한다.
	if(trx_sensor_for_dp173_result){
		printf("PASS\n");
	}else{
		printf("FAIL\n");
	}
	...
	for(i=0;i<8;i++){
		...
		for(bit=0;bit<8;bit++){
			if(trx_short_bits[i*8+bit] != 255){		// 사용하는 핀만 채널명과 판정결과 출력
				...
				if(((temp[i]>>bit) & 0x01) != ((trx_sensor_for_dp173_spec[i]>>bit) & 0x01))			// 데이터와 spec을 비교
					printf(" NG ");
				else
					printf(" OK ");
			}
...
int synaptics_04_sensor_speed_check(){
...
//	printf("temp1 : %s\ntemp2 : %s\n", temp1, temp2);		// OK
	...
	//send_command_for_dp173_multi_payload(PRODUCTION_TEST, payload, sizeof(payload));	
	send_command_for_dp173_multi_payload_output_buf_int16_t(PRODUCTION_TEST, payload, sizeof(payload), sensor_for_dp173);	
	...
	for (i = 0; i < SENSOR; i++){
		...
		value = (float)(sensor_for_dp173[i]) / 1000.f;
...
int synaptics_04_extended_high_resist_check(){	
	...
	send_command_for_dp173_multi_payload_output_buf(PRODUCTION_TEST, payload, sizeof(payload), fullraw_for_dp173);	
	...
	Baseline = fullraw_for_dp173;		// unsigned short 를 short 배열에다 담았다.
	{
		int i;
		for(i=0;i<(NUM_RX*NUM_TX);i++){
			Delta[i] = Baseline[i] - p_ref_frame_for_dp173[i];
		}
	}
#if 1
...
int synaptics_04_adc_range_check(){	
	int i;										// spec : TABLE
	int res = 1;
	...
	send_command_for_dp173_multi_payload_output_buf(PRODUCTION_TEST, payload, sizeof(payload), adcrange_for_dp173);	
	...
	for (row = 0; row < IMAGE_ROW; row++)
    {
        ...
        {
            if((adcrange_for_dp173[offset + col] > (uint16_t)(l_limit.ADC_RANGE_MAX)[row][col]) || (adcrange_for_dp173[offset + col] < (uint16_t)(l_limit.ADC_RANGE_MIN)[row][col])){
				if(DEBUG_MODE)
					printf("!");
                res = 0;
            }
			...
			if(DEBUG_MODE)
				printf("%4d ", adcrange_for_dp173[offset + col]);

			#if 0
			printf("[%d:rawdata is %hu, ", offset + col, adcrange_for_dp173[offset + col]);
...
int synaptics_04_full_raw_cap_check(){
	int row, col;								// spec : TABLE
    int offset;
	int res = 1;
	...
	for (row = 0; row < IMAGE_ROW; row++)
    {
        ...
        for (col = 0; col < IMAGE_COL; col++)
        {
            if((fullraw_for_dp173[offset + col] > (uint16_t)(l_limit.full_raw_cap_MAX)[row][col]) || (fullraw_for_dp173[offset + col] < (uint16_t)(l_limit.full_raw_cap_MIN)[row][col])){
			...
			if(DEBUG_MODE)
				printf("%4hu ", fullraw_for_dp173[offset + col]);

			#if 0
			printf("[%d:rawdata is %hu, ", offset + col, fullraw_for_dp173[offset + col]);
...
int synaptics_04_full_raw_cap_h_check(){
	int row, col;								// spec : TABLE
    int offset;
	int res = 1;
	...
	for (row = 0; row < IMAGE_ROW; row++)
    {
        ...
        for (col = 0; col < IMAGE_COL; col++)
        {
			if(col == IMAGE_COL -1){
				...
				fullraw_for_dp173_h_for_dp173[offset + col] = abs(fullraw_for_dp173[offset + col] - fullraw_for_dp173[offset + col + 1]);
				
				if((fullraw_for_dp173_h_for_dp173[offset + col] > (int16_t)(l_limit.full_raw_cap_h)[row][col])){
					if(DEBUG_MODE)
						printf("!");
                	res = 0;
            	}
				...
				if(DEBUG_MODE)
					printf("%4hd ", fullraw_for_dp173_h_for_dp173[offset + col]);

				#if 0
				printf("[rawdata is %hd, spec is %hd]\n", fullraw_for_dp173_h_for_dp173[offset + col], (int16_t) l_limit.full_raw_cap_h[row][col]);
				#endif
			}
...
int synaptics_04_full_raw_cap_v_check(){
	int row, col;								// spec : TABLE
    int offset1, offset2;
	int res = 1;
	...
	for (row = 0; row < IMAGE_ROW; row++)
    {
    	...
    	for (col = 0; col < IMAGE_COL; col++)
    	{
			fullraw_for_dp173_v_for_dp173[offset1 + col] = abs(fullraw_for_dp173[offset1 + col] - fullraw_for_dp173[offset2 + col]);
			
			if((fullraw_for_dp173_v_for_dp173[offset1 + col] > (int16_t)(l_limit.full_raw_cap_v)[row][col])){
				if(DEBUG_MODE)
					printf("!");
   	            res = 0;
   	        }
			...
			if(DEBUG_MODE)
				printf("%4hd ", fullraw_for_dp173_v_for_dp173[offset1 + col]);

			#if 0
			printf("[rawdata is %hd, spec is %hd]\n", fullraw_for_dp173_v_for_dp173[offset1 + col], (int16_t) l_limit.full_raw_cap_v[row][col]);
			#endif
		}
...
int synaptics_04_hybrid_abs_raw_cap_check(){
	int i;										// spec : ARRAY
	int res = 1;

	u8 payload[] = {TEST_ABSRAW};
	send_command_for_dp173_multi_payload_output_buf_for_hybrid_abs(PRODUCTION_TEST, payload, sizeof(payload), hybrid_abs_with_cbc_for_dp173);
	...
	for (i = 0; i < HYBRID_ABS_RX; i++){
		if((hybrid_abs_with_cbc_for_dp173[i] > (uint32_t)(l_limit.hybrid_raw_cap_RX_MAX)[0][i]) || (hybrid_abs_with_cbc_for_dp173[i] < (uint32_t)(l_limit.hybrid_raw_cap_RX_MIN)[0][i])){
			printf("!");
            res = 0;
        }

		printf("%4d ", hybrid_abs_with_cbc_for_dp173[i]);

		#if 0
		printf("[%d:rawdata is %u, ", i, hybrid_abs_with_cbc_for_dp173[i]);
		//printf("[%d:rawdata is 0x%x, ", i, hybrid_abs_with_cbc_for_dp173[i]);
	...
	for (i = 0; i < HYBRID_ABS_TX; i++){
		if((hybrid_abs_with_cbc_for_dp173[HYBRID_ABS_RX+i] > (uint32_t)(l_limit.hybrid_raw_cap_TX_MAX)[0][i]) || (hybrid_abs_with_cbc_for_dp173[HYBRID_ABS_RX+i] < (uint32_t)(l_limit.hybrid_raw_cap_TX_MIN)[0][i])){
			printf("!");
            res = 0;
        }

		printf("%4d ",  hybrid_abs_with_cbc_for_dp173[HYBRID_ABS_RX+i]);

		#if 0
		printf("[%d:rawdata is %u, ", i, hybrid_abs_with_cbc_for_dp173[HYBRID_ABS_RX+i]);
...
// test_noise_for_dp173
// PRODUCTION_TEST(TEST_NOISE)
#define NOISE_DEBUG    0
int synaptics_04_noise_check(){	
	int i;										// spec : -10 ~ 10
	int res = 1;
	...
	//send_command_for_dp173_multi_payload(PRODUCTION_TEST, payload, sizeof(payload));				
	//send_command_for_dp173_multi_payload_output_buf(PRODUCTION_TEST, payload, sizeof(payload), noise_for_dp173);				
	send_command_for_dp173_multi_payload_output_buf_int16_t(PRODUCTION_TEST, payload, sizeof(payload), noise_for_dp173);
	...
	for (i = 0; i < NOISE; i++){
		...
		if((noise_for_dp173[i] > max) || (noise_for_dp173[i] < min)){
			if(DEBUG_MODE)
				printf("!");
            res = 0;
        }
		...
		if(DEBUG_MODE)
			printf("%4d ", noise_for_dp173[i]);	

		#if 0
		printf("[%d:rawdata is %hd, ", i, noise_for_dp173[i]);
...
int synaptics_04_customer_id_check(){
...
int synaptics_04_hsync_vsync_check(){
	int res;									// PASS/FAIL
...
/////////////////////////////////////////////////////////////////////////////////////////
// 190517 renew : send_command_for_dp173, send_command_for_dp173_delay, send_command_for_dp173_multi_payload를 사용
// 1. send_command_for_dp173 : 기본적인 커맨드
// 2. send_command_for_dp173_delay : 읽고 쓸때 200ms 정도 딜레이가 필요한 커맨드에 사용
// 3. send_command_for_dp173_multi_payload : Production Test, Read Flash 등, 인자를 넘겨주는 커맨드에 널리 사용

#define BASIC_DEBUG		1	
void send_command_for_dp173(u8 addr){	
	int rc, len, len_cnt;
...
#else
	spi_func_for_dp173(wbuf+1, synap_rbuf_for_dp173, 1);
#endif
	usleep(10000);
...
#else
	spi_func_for_dp173(wbuf+1, synap_rbuf_for_dp173, 199);
#endif
	usleep(10000);
	
#ifndef VFOS_SITE_VIETNAM_2
//#ifdef VFOS_SITE_VIETNAM_2			// VIETNAM1 에서 잘도는지 TEST 목적
	rc = read(mux_device, synap_rbuf_for_dp173, 199);			// host take 199 bytes
#endif	

	usleep(10000);
	len = (synap_rbuf_for_dp173[2]) | (synap_rbuf_for_dp173[3] << 8);	// valid length
	...
	if(4+len <= 199){		// 버퍼를 안넘는다면 그대로 수행
		if(rc > -1){
        	...
        	for(i = 0; i < 4+len; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    		{
    		    if(!(i % 6))
    		    	printf("\n");
    		    printf("[%d:0x%X] ",i,*(synap_rbuf_for_dp173+i));
    		}
    		printf("\n");
			#endif
		}
		...
		if(rc > -1){
			...
			do{
				for(i = 0; i < 199; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    			{
					...
					if(*(synap_rbuf_for_dp173+i) == 0x5A)	{
						exit = 1;
						break;
					}
    			}
...
void send_command_for_dp173_delay(u8 addr){	// FOR ENTER BOOTLOADER MODE
	int rc, len, len_cnt;
	int arg;	// DUMMY ( NOT USED )
//	u8 wbuf[100] = {0xff, };
	u8 wbuf[200] = {0xff, };
...
void send_command_for_dp173_multi_payload(u8 addr, u8 *payload, int payload_len)
{
	int rc, len;
	int arg;	// DUMMY ( NOT USED )
//	u8 wbuf[100] = {0xff, };
...
#else
	spi_func_for_dp173(wbuf+1, synap_rbuf_for_dp173, 3+payload_len);
#endif
	usleep(200000);
...
#if 1
	if(synap_rbuf_for_dp173[1] == 0x00){		// redo
		goto REDO;
	}
#endif
		...
		if(rc > -1){
			...
			do{
				...
				for(i = 0; i < 199; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    			{
					...
					if(*(synap_rbuf_for_dp173+i) == 0x5A)	{			// 현제 값이 0x5A 라면, 패킷이 끝났다는 의미인지, 그냥 값인지를 알아야 한다.
						if(*(synap_rbuf_for_dp173+198) == 0x5A){			// 정말로 패킷이 다 끝난건지 확인해본다
							//printf("\n 0x5A detected\n");
							exit = 1;
							break;
						}
				...
				is_first_loop = 1;
				memset(synap_rbuf_for_dp173, 0xAA, 200);	// 초기화 시킨다.
				wbuf[1] = 0xff;
#ifndef VFOS_SITE_VIETNAM_2
//#ifdef VFOS_SITE_VIETNAM_2		// VIETNAM1 에서 잘도는지 TEST 목적
				write(mux_device, wbuf, 200);	// touch-ic ready to send 199 bytes
...
void send_command_for_dp173_multi_payload_output_buf(u8 addr, u8 *payload, int payload_len, uint16_t *output_buf)
{
	int rc, len;
	u8 wbuf[100] = {0xff, };
        	...
        	for(i = 0; i < 4+len; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    		{
				...
				if((i%2) == 1){				// 값을 저장
					if((i!=1) && (i!=3)){	// i==1일 땐 응답코드, i==3일땐 길이이다.
						output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+i) << 8 | *(synap_rbuf_for_dp173+i-1);					
		...
		if(rc > -1){
			...
			do{
				...
				for(i = 0; i < 199; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    			{
					...
					////////////// OUTPUT_BUF 저장 알고리즘 ///////////////	
					if(*(synap_rbuf_for_dp173 + 1) == 0x01)		{		// OK 패킷일때(0xA5 0x01)
						if((i%2) == 1){				// 값을 저장
							if((i!=1) && (i!=3)){	// i==1일 땐 응답코드, i==3일땐 길이이다.
								output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+i) << 8 | *(synap_rbuf_for_dp173+i-1);					
							...
							save_198 = *(synap_rbuf_for_dp173+198);
							use_save_198 = 1;	
					...
					}else if (*(synap_rbuf_for_dp173 + 1) == 0x03){		// CONTINUE 패킷일때(0xA5 0x03)
						...
						if(use_save_198 == 1){
							if(i==2){
								...
								output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+2) << 8 | save_198;					
								...
								output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+198) << 8 | *(synap_rbuf_for_dp173+197);					
...
void send_command_for_dp173_multi_payload_output_buf_int16_t(u8 addr, u8 *payload, int payload_len, int16_t *output_buf)
{
	int rc, len;
	u8 wbuf[100] = {0xff, };
...
void send_command_for_dp173_multi_payload_output_buf_uint8_t(u8 addr, u8 *payload, int payload_len, uint8_t *output_buf)
{
	int rc, len;
	u8 wbuf[100] = {0xff, };
	...
	if(4+len <= 199){		// 버퍼를 안넘는다면 그대로 수행
		if(rc > -1){
        	...
        	for(i = 0; i < 4+len; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    		{
				...
				if(i>=4){
					output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+i);
...
// 고정길이 216 bytes를 읽는다.
void send_command_for_dp173_multi_payload_output_buf_for_hybrid_abs(u8 addr, u8 *payload, int payload_len, uint32_t *output_buf)
{
	int rc, len;
	u8 wbuf[100] = {0xff, };
	...
	if(rc > -1){
		...
		for(i = 0; i < 199; i++)	// [A5] [RESPONSE] [LEN1] [LEN2] ................
    	{
			...
			if((i>=7) && ((i-7)%4 == 0)){				// 값을 저장
				output_buf[output_buf_idx] = *(synap_rbuf_for_dp173+i) << 24 | *(synap_rbuf_for_dp173+i-1) << 16 | *(synap_rbuf_for_dp173+i-2) << 8 | *(synap_rbuf_for_dp173+i-3);					
				#if HYBRID_ABS_DEBUG
				printf("[%d] rawdata is 0x%x\n", output_buf_idx, output_buf[output_buf_idx]);
				#endif
				output_buf_idx++;
		...
		// 임시저장
		save[0] = *(synap_rbuf_for_dp173+196);
		save[1] = *(synap_rbuf_for_dp173+197);
		save[2] = *(synap_rbuf_for_dp173+198);
...
//		for(i = 0; i < 23; i++)
		for(i = 0; i < 39; i++)
		{
			...
			if((i==0) | (i==1)){
				...
				#endif
				output_buf[output_buf_idx] = (*(synap_rbuf_for_dp173+2) << 24) | ((save[2]) << 16) | ((save[1]) << 8) | (save[0]);
				#if HYBRID_ABS_DEBUG
				printf("[%d] rawdata is 0x%x\n", output_buf_idx, output_buf[output_buf_idx]);
				#endif
				output_buf_idx++;
