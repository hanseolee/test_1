...
/* main structure for model DP173 */
model_dp173_t	*model_dp173_p;
...
void display_on_by_command_for_dp173(void);
void display_off_by_command_for_dp173(void);

/*
 * Name : init_variable_for_dp173
 * Description : Initialize variables to make initial condition.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value :
 */
void init_variable_for_dp173(model_dp173_t *dp173_p)
{
	int ch_cnt = 0;

	FUNC_BEGIN();

	for (ch_cnt = 0;ch_cnt < DP173_POCB_WRITE_CHANNEL_NUM;ch_cnt++)
	{
		dp173_p->pocb_status.pocb_init_status[ch_cnt] = POCB_STATUS_NO_READ;
		dp173_p->pocb_status.pocb_cur_status[ch_cnt] = POCB_STATUS_NO_READ;
	}
	dp173_p->pocb_status.flag_pocb_changed = POCB_STATUS_NO_CHANGE;
	dp173_p->cur_image_num = 0;

	FUNC_END();
}

/*
 * Name : put_info_for_dp173_thread
 * Description : Set information which are needed to set DP173 thread, it is called by main() function.
 * Parameters :
 * 		model_dp173_info_t *info_p
 * Return value :
 */
void put_info_for_dp173_thread(model_dp173_info_t *info_p)
{
	model_dp173_info_t *dp173_info_p = &model_dp173_p->model_dp173_info;

	FUNC_BEGIN();
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);	
	printf("TEST dp173_info_p->key_dev \n");
	dp173_info_p->key_dev = info_p->key_dev;														// 190924 LWG CRASH HERE
	printf("TEST info_p->key_dev \n");
	dp173_info_p->key_dev = info_p->key_dev;														// 190924 LWG CRASH HERE
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);	
	dp173_info_p->model_dp173_id = info_p->model_dp173_id;
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);	
	dp173_info_p->next_model_id = info_p->next_model_id;
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);	
	dp173_info_p->buf_index = info_p->buf_index;
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);	
//	dp173_info_p->image_directory = info_p->image_directory;
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	memcpy(dp173_info_p->display_image_file_name,info_p->display_image_file_name,MAX_DISPLAY_IMAGE_NUM * MAX_FILE_NAME_LENGTH);
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	memcpy(&dp173_info_p->dp173_manager, &info_p->dp173_manager,sizeof(MODEL_MANAGER));
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	memcpy(&dp173_info_p->version_info,&info_p->version_info,sizeof(vfos_version_info_t));
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);

	/* Convert image directory character to string */
	strcpy(dp173_info_p->display_image_dir,&dp173_info_p->dp173_manager.dir);
printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
...
/*
 * Name : get_info_for_dp173_thread
 * Description : Get information which are needed to set DP173 thread.
 * Parameters :
 * Return value :
 */
int get_info_for_dp173_thread(void)
{
	FUNC_BEGIN();
...
/*
 * Name : parsing_pattern_command_and_write_for_dp173
 ...
 */
int parsing_pattern_command_and_write_for_dp173(char *parsing_file_name_p, char *parsing_code_name_p)
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[MAX_CODE_NAME_STRING_LENGTH];
	mipi_write_data_t *mipi_data = &model_dp173_p->g_mipi_write;

	FUNC_BEGIN();

	memset(code_file_name,0,sizeof(code_file_name));
	sprintf(code_file_name,"%s/%s/%s", SD_CARD_DIR_PATH, DP173_CONFIG_DIR_NAME, DP173_CONFIG_FILE_NAME);
	...
	ret = parsing_pattern_write_command_for_dp173(code_file_name, code_name, mipi_data);
	if (ret < 0)
	{
		DERRPRINTF("parsing_pattern_write_command_for_dp173\n");
		return ret;
	}
	else
	{
...
/*
 * Name : parsing_pattern_write_command_for_dp173
...
// Replaceable parsing_pattern_command_and_write_for_dp173();
int parsing_pattern_write_command_for_dp173(char *parsing_file_name_p, char *parsing_code_name_p, mipi_write_data_t *mipi_data_p)
{
	int ret = -1;
    FILE *fp;
    char *token = NULL;
...
/*
 * Name : gray_scan_thread_for_dp173
 * Description : Thread for gray scan.
 * Parameters :
 * 		void *arg : arguments for gray scan thread.
 * Return value : NULL
 */
void *gray_scan_thread_for_dp173(void *arg)
{
	int ret = 0;
	model_dp173_t *dp173_p = (model_dp173_t *)arg;
	pthread_mutex_t	*mutex_p = &dp173_p->gray_scan_thread_mutex;
	int thread_loop = 0;
	char code_name[20];
	int gray_value = 0;
	...
	/* set flag */
	dp173_p->flag_run_gray_scan_thread = true;
	dp173_p->flag_finish_gray_scan_thread = false;
	...
	while (thread_loop)
	{
		...
		{
			gray_value++;
			if (gray_value >= DP173_GRAY_SCAN_MAX_VALUE)
			{
				gray_value = DP173_GRAY_SCAN_MAX_VALUE;
				positive_dir = false;
			}
		}
		else
		{
			gray_value--;
			if (gray_value <= DP173_GRAY_SCAN_MIN_VALUE)
			{
				positive_dir = true;
			}
		}
		/* set gray_value to mipi data */
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_RED_BYTE_1_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_GREEN_BYTE_1_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_BLUE_BYTE_1_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_NO_INFO_BYTE_1_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_RED_BYTE_2_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_GREEN_BYTE_2_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_BLUE_BYTE_2_DATA_NUM] = (unsigned char)gray_value;
		mipi_data->data_buf[mipi_data->reg_cnt - 1][DP173_GRAY_SCAN_NO_INFO_BYTE_2_DATA_NUM] = (unsigned char)gray_value;
		...
		if (dp173_p->flag_run_gray_scan_thread == false)
		{
			thread_loop = false;
		}
	...
	ret = parsing_pattern_command_and_write_for_dp173(code_file_name,code_name);
	if (ret < 0)
	{
		DERRPRINTF("parsing_pattern_command_and_write_for_dp173\n");
	}
	
	usleep(10000);	/* 10ms delay - need to check if it is needed */

	/* set flag_finish_gray_scan_thread */
	dp173_p->flag_finish_gray_scan_thread = true;
...
/*
 * Name : start_gray_scan_thread_for_dp173
 * Description : Start gray_scan thread.
 * Parameters :
 * Return value :
 * 		error value
 */
int start_gray_scan_thread_for_dp173(void)
{
	int ret = 0;
	...
	/* initialize mutex */
	ret = pthread_mutex_init(&model_dp173_p->gray_scan_thread_mutex,NULL);
	if (ret < 0)
	{
	...
	ret = pthread_create(&model_dp173_p->id_gray_scan_thread, NULL, gray_scan_thread_for_dp173, (void *)(model_dp173_p));
	if (ret < 0)
	{
...
/*
 * Name : wait_for_finish_gray_scan_thread_for_dp173
 * Description : wait for finishing gray_scan thread.
 * Parameters :
 * Return value :
 */
void wait_for_finish_gray_scan_thread_for_dp173(void)
{
	model_dp173_t *dp173_p = model_dp173_p;	/* no way to avoid using global variable */
	pthread_mutex_t	*mutex_p = &dp173_p->gray_scan_thread_mutex;
	int wait_loop = 0;
	...
	/* lock */
	pthread_mutex_lock(mutex_p);
	dp173_p->flag_run_gray_scan_thread = false;
	/* unlock */
	pthread_mutex_unlock(mutex_p);
	...
	while ((wait_loop == true) && (timeout_cnt > 0))
	{
		...
		/* lock */
		pthread_mutex_lock(mutex_p);
		if (dp173_p->flag_finish_gray_scan_thread == true)
		{
			wait_loop = false;
		}
		/* unlock */
...
/*
 * Name : dimming_thread_for_dp173
 * Description : Thread for dimming.
 * Parameters :
 * 		void *arg : arguments for dimming thread.
 * Return value : NULL
 */
void *dimming_thread_for_dp173(void *arg)
{
	int ret = 0;
	model_dp173_t *dp173_p = (model_dp173_t *)arg;
	pthread_mutex_t	*mutex_p = &dp173_p->dimming_thread_mutex;
	int thread_loop = 0;
	char code_name[20];
	int dbv_value = 0;
	int positive_dir = true;
	...
	/* set flag */
	dp173_p->flag_run_dimming_thread = true;
	dp173_p->flag_finish_dimming_thread = false;

	/* set variable */
	thread_loop = true;
	positive_dir = false;				/* direction to down */
	dbv_value = DP173_DBV_MAX_VALUE;	/* set dbv_value to start */
...
//		DPRINTF("=============================dbv_value=(0x%04x)=============================\n", dbv_value);
		...
		{
			dbv_value+=4;
			if (dbv_value >= DP173_DBV_MAX_VALUE)
			{
				dbv_value = DP173_DBV_MAX_VALUE;
				positive_dir = false;
			}
		}
		else
		{
			dbv_value-=4;
			if (dbv_value <= DP173_DBV_MIN_VALUE)
			{
				dbv_value = DP173_DBV_MIN_VALUE;
				positive_dir = true;
			}
		}
		/* set dbv_value to mipi data */
		mipi_data->data_buf[mipi_data->reg_cnt - 1][2] = (unsigned char)((dbv_value >> 8) & DP173_DBV_HIGH_8BITS_FILTER);
		...
		if (dp173_p->flag_run_dimming_thread == false)
		{
			thread_loop = false;
		}
	...
	dp173_p->flag_finish_dimming_thread = true;
...
/*
 * Name : start_dimming_thread_for_dp173
 * Description : Start dimming thread.
 * Parameters :
 * Return value :
 * 		error value
 */
int start_dimming_thread_for_dp173(void)
{
	int ret = 0;
	...
	/* initialize mutex */
	ret = pthread_mutex_init(&model_dp173_p->dimming_thread_mutex,NULL);
	if (ret < 0)
	{
		DERRPRINTF("pthread_mutex_init(errno=%d)\n", errno);
		FUNC_END();
	...
	ret = pthread_create(&model_dp173_p->id_dimming_thread, NULL, dimming_thread_for_dp173, (void *)(model_dp173_p));
	if (ret < 0)
	{
		DERRPRINTF("pthread_create(errno=%d)\n", errno);
		FUNC_END();
...
/*
 * Name : wait_for_finish_dimming_thread_for_dp173
 * Description : wait for finishing dimming thread.
 * Parameters :
 * Return value :
 */
void wait_for_finish_dimming_thread_for_dp173(void)
{
	model_dp173_t *dp173_p = model_dp173_p;	/* no way to avoid using global variable */
	pthread_mutex_t	*mutex_p = &dp173_p->dimming_thread_mutex;
	int wait_loop = 0;
	...
	pthread_mutex_lock(mutex_p);
	/* set flag */
	dp173_p->flag_run_dimming_thread = false;
	/* unlock */
	pthread_mutex_unlock(mutex_p);
	...
	while ((wait_loop == true) && (timeout_cnt > 0))
	{
		...
		/* lock */
		pthread_mutex_lock(mutex_p);
		if (dp173_p->flag_finish_dimming_thread == true)
		{
			wait_loop = false;
		}
		/* unlock */
...
/*
 * Name : dsc_roll_thread_for_dp173
 * Description : Thread for dsc_roll.
 * Parameters :
 * 		void *arg : arguments for dsc_roll thread.
 * Return value : NULL
 */
void *dsc_roll_thread_for_dp173(void *arg)
{
	model_dp173_t *dp173_p = (model_dp173_t *)arg;
	model_dp173_info_t *dp173_info_p = &dp173_p->model_dp173_info;
	pthread_mutex_t	*mutex_p = &dp173_p->dsc_roll_thread_mutex;
	int thread_loop = 0;
	...
	/* set flag */
	dp173_p->flag_run_dsc_roll_thread = true;
	dp173_p->flag_finish_dsc_roll_thread = false;

	/* set variable */
	dsc_roll_pic_num = dp173_p->special_pattern_mode.dsc_pattern_mode.dsc_roll_pic_num;
	dsc_roll_name_string_p = dp173_p->special_pattern_mode.dsc_pattern_mode.dsc_roll_name_string;
	thread_loop = true;
	dsc_roll_cnt = 0;

	/* get dsc_roll file number and file names */
	memset(file_name_string,0,sizeof(file_name_string));
	dsc_roll_file_num = parsing_dsc_roll_test_file_name(dp173_info_p->display_image_dir,dsc_roll_pic_num,dsc_roll_name_string_p,file_name_string);
	/* dsc_roll test directory */
	memset(dsc_roll_test_dir,0,sizeof(dsc_roll_test_dir));
	sprintf(dsc_roll_test_dir,"%c/%02d%s",dp173_info_p->dp173_manager.dir,dsc_roll_pic_num,DSC_ROLL_TEST_DIR_STRING);
	...
	while (thread_loop)
	{
		...
		if (dp173_p->flag_run_dsc_roll_thread == false)
		{
			thread_loop = false;
		}
	...
	dp173_p->flag_finish_dsc_roll_thread = true;
...
/*
 * Name : start_dsc_roll_thread_for_dp173
 * Description : Start dsc_roll thread.
 * Parameters :
 * Return value :
 * 		error value
 */
int start_dsc_roll_thread_for_dp173(void)
{
	int ret = 0;
	...
	/* initialize mutex */
	ret = pthread_mutex_init(&model_dp173_p->dsc_roll_thread_mutex,NULL);
	if (ret < 0)
	{
		DERRPRINTF("pthread_mutex_init(errno=%d)\n", errno);
		FUNC_END();
	...
	ret = pthread_create(&model_dp173_p->id_dsc_roll_thread, NULL, dsc_roll_thread_for_dp173, (void *)(model_dp173_p));
	if (ret < 0)
	{
		DERRPRINTF("pthread_create(errno=%d)\n", errno);
		FUNC_END();
...
/*
 * Name : wait_for_finish_dsc_roll_thread_for_dp173
 * Description : wait for finishing dsc_roll thread.
 * Parameters :
 * Return value :
 */
void wait_for_finish_dsc_roll_thread_for_dp173(void)
{
	model_dp173_t *dp173_p = model_dp173_p;	/* no way to avoid using global variable */
	pthread_mutex_t	*mutex_p = &dp173_p->dsc_roll_thread_mutex;
	int wait_loop = 0;
	...
	pthread_mutex_lock(mutex_p);
	/* set flag */
	dp173_p->flag_run_dsc_roll_thread = false;
	wait_loop = true;
	...
	while ((wait_loop == true) && (timeout_cnt > 0))
	{
		...
		/* lock */
		pthread_mutex_lock(mutex_p);
		if (dp173_p->flag_finish_dsc_roll_thread == true)
		{
			wait_loop = false;
		}
		/* unlock */
...
/*
 * Name : control_special_pattern_mode_for_dp173
 ...
 * Return value :
 */
//int control_special_pattern_mode_for_dp173(model_dp173_t *dp173_p, int type_flag, char *code_file_name_p, unsigned int pattern_mode)
int control_special_pattern_mode_for_dp173(model_dp173_t *dp173_p, int type_flag, char *code_file_name_p, unsigned long long pattern_mode)
{
	int ret = 0;
	int matched = 0;
	...
	if (type_flag == SPECIAL_PATTERN_PREVIOUS_MODE)
	{
		...
		{
			wait_for_finish_gray_scan_thread_for_dp173();
			matched = false;
		}
		else if (pattern_mode & DIMMING_MODE)	/* wait for finishing thread */
		{
			wait_for_finish_dimming_thread_for_dp173();
			matched = false;
		}
		else if (pattern_mode & DSC_ROLL_MODE)	/* wait for finishing thread */
		{
			wait_for_finish_dsc_roll_thread_for_dp173();
			matched = false;
		}
		//181211
	...
	else if (type_flag == SPECIAL_PATTERN_CURRENT_MODE)
	{
		...
		else if (pattern_mode & SLEEP_MODE)
		{
			...
			matched = true;
//			need_to_display_off = true;	/* this will be set by command in dp173.config file for Sleep mode, but display on command has to be set in F/W */
		}
		else if (pattern_mode & BRIGHT_LINE_MODE)
		{
		...
		else if (pattern_mode & GRAY_MODE)	/* new thread will run */
		{
			...
			ret = start_gray_scan_thread_for_dp173();
			if (ret < 0)
			{
		...
		{
			ret = start_dimming_thread_for_dp173();
			if (ret < 0)
			{
		...
		{
			ret = start_dsc_roll_thread_for_dp173();
			if (ret < 0)
			{
	...
	if (matched == true)
	{
		...
		{
			display_off_by_command_for_dp173();
		}
		/* display on flag as DISPLAY_ON code has to be set after display PTN */
		if (need_to_display_on == true)
		{
			dp173_p->flag_need_to_display_on = true;
		}

		/* parsing pattern command to get mipi data to be written */
		ret = parsing_pattern_write_command_for_dp173(code_file_name_p,&code_name[0],mipi_data);
		if (ret < 0)
		{
			DERRPRINTF(" parsing_pattern_write_command_for_dp173\n");
			mipi_data_set = false;
...
/*
 * Name : send_current_test_result_to_uart_for_dp173
 ...
 * Return value :
 */
void send_current_test_result_to_uart_for_dp173(current_test_result_t (*result_p)[MAX_VOLT_NUM_VDDD], int number_of_pattern, int pattern_num, int ch_num)
{
...
/*
 * Name : send_display_info_to_uart_for_dp173
 ...
 * 		model_dp173_t *dp173_p : dp173 main structure.
 * Return value :
 */
void send_display_info_to_uart_for_dp173(int key_value, model_dp173_t *dp173_p)
{
	unsigned char uart_buf[MAX_PACKET];
	MODEL_MANAGER *manager_p = &dp173_p->model_dp173_info.dp173_manager;
	...
	/* uart command */
	memset(uart_buf, 0, MAX_PACKET);
	uart_buf[4] = dp173_p->cur_image_num - 1;
...
int read_key_input_for_dp173(void)
{
	...
	if(read (dp173_info_p->key_dev, &ev_key, sizeof (ev_key)))
	{
	    if(ev_key.code != 0)
	    {
	        if(ev_key.value)
...
/*
 * Name : display_module_on_for_dp173
 ...
 * Return value : 
 */
void display_module_on_for_dp173(model_dp173_t *dp173_p, int model_index)
{
	char comm[100] ={0,};
	...
	dp173_p->flag_already_module_on = true;
	dp173_p->cur_image_num = 0;
...
/*
 * Name : display_module_off_for_dp173
 ...
 * Return value : 
 */
void display_module_off_for_dp173(model_dp173_t *dp173_p,int model_index)
{
	char comm[100] ={0,};
	...
	dp173_p->flag_already_module_on = false;
	dp173_p->cur_image_num = 0;
...
void aod_on_by_command_for_dp173(void)
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[20];
...
/*
 * Name : display_on_by_command_for_dp173
 * Description : Display ON by command on DP173.
 * Parameters :
 * Return value : 
 */
void display_on_by_command_for_dp173(void)
...
/*
 * Name : display_off_by_command_for_dp173
 * Description : Display OFF by command on DP173.
 * Parameters :
 * Return value : 
 */
void display_off_by_command_for_dp173(void)
...
/*
 * Name : otp_read_for_dp173
 * Description : Read OTP data from DP173.
 * Parameters :
 * 		unsigned char (*otp_value_p)[] : return value for OTP read.
 * Return value : error
 */
int otp_read_for_dp173(unsigned char (*otp_value_p)[DP173_OTP_READ_LENGTH])
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[20];
	mipi_write_data_t *mipi_write_data = &model_dp173_p->g_mipi_write;
	mipi_read_data_t *mipi_read_data = &model_dp173_p->g_mipi_read;
	unsigned char read_data[DP173_OTP_READ_LENGTH];
	int read_ch_num = 0;
	int read_ch_cnt = 0;
	int read_len = 0;
	...
	ret = parsing_pattern_write_command_for_dp173(code_file_name,code_name,mipi_write_data);
	if (ret < 0)
	{
		DERRPRINTF("parsing_pattern_write_command_for_dp173\n");
	}
...
//		return ret;
	...
	{
		read_ch_num = DP173_OTP_READ_CHANNEL_NUM;
...
//					return ret;
			...
			read_len = DP173_OTP_READ_LENGTH;
...
int dbv_read_for_dp173(unsigned char (*otp_value_p)[DP173_OTP_READ_LENGTH]){
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[20];
	mipi_write_data_t *mipi_write_data = &model_dp173_p->g_mipi_write;
...
/*
 * Name : crc_read_for_dp173
 * Description : Read CRC data from DP173.
 * Parameters :
 * 		unsigned char (*crc_value_p)[] : return value for CRC read.
 * Return value : error
 */
int crc_read_for_dp173(unsigned char (*crc_write_value_p)[DP173_CRC_READ_LENGTH], unsigned char (*crc_read_value_p)[DP173_CRC_READ_LENGTH])
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[50];
	mipi_write_data_t *mipi_write_data = &model_dp173_p->g_mipi_write;
	mipi_read_data_t *mipi_read_data = &model_dp173_p->g_mipi_read;
	unsigned char read_data[DP173_CRC_READ_LENGTH];		
	int read_ch_num = 0;
	int read_ch_cnt = 0;
	int read_len = 0;
	int i;
	...
	if (ret < 0)
	{
		...
		read_ch_num = DP173_CRC_READ_CHANNEL_NUM;
		...
		for (read_ch_cnt = 0;read_ch_cnt < read_ch_num;read_ch_cnt++)
		{
			...
			else
			{
				...
				for(i=0;i<DP173_CRC_READ_LENGTH;i++){
...
/*
 * Name : pocb_read_for_dp173
 * Description : Read POCB data from DP173 to check whether POCB is enabled or not.
 * Parameters :
 * 		unsigned char (*pocb_status_p)[] : return value of POCB status.
 * Return value : error
 */
int pocb_read_for_dp173(unsigned char (*pocb_status_p)[DP173_POCB_READ_LENGTH])
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[MAX_CODE_NAME_STRING_LENGTH];
	mipi_read_data_t *mipi_data = &model_dp173_p->g_mipi_read;
	//unsigned char read_data[DP173_POCB_READ_LENGTH];
...
//		return ret;
	...
	{
		read_ch_num = DP173_POCB_READ_CHANNEL_NUM;
	
		if(read_ch_cnt == 0)
			model_dp173_p->ch1_pocb_status = 2;			// FAIL
		else
			model_dp173_p->ch2_pocb_status = 2;			// FAIL
...
//					return ret;
			...
			usleep(3000);	/* 3ms delay */
			read_len = DP173_POCB_READ_LENGTH;
...
//				pocb_status = POCB_STATUS_NO_READ;	/* default set */
				...
				if ( !((read_data[0] & (1 << 0)) >> 0) )
				{
					...
					if(read_ch_cnt == 0){
						if(first_pocb_success_ch1 == 1){
							pocb_status = POCB_STATUS_ON;
							printf("%d                >>> LWG <<< [%s %d] %s CALL ====== \n", pocb_status, __FILE__, __LINE__, __FUNCTION__);
							model_dp173_p->ch1_pocb_status = 1;			// PASS
						}else{
							pocb_status = POCB_STATUS_OFF;
							printf("%d                >>> LWG <<< [%s %d] %s CALL ====== \n", pocb_status, __FILE__, __LINE__, __FUNCTION__);
							//model_dp173_p->ch1_pocb_status = 2;			// FAIL
						...
						if(first_pocb_success_ch2 == 1){
							pocb_status = POCB_STATUS_ON;
							printf("%d                >>> LWG <<< [%s %d] %s CALL ====== \n", pocb_status, __FILE__, __LINE__, __FUNCTION__);
							model_dp173_p->ch2_pocb_status = 1;			// PASS
						}else{
							pocb_status = POCB_STATUS_OFF;
							printf("%d                >>> LWG <<< [%s %d] %s CALL ====== \n", pocb_status, __FILE__, __LINE__, __FUNCTION__);
							//model_dp173_p->ch2_pocb_status = 2;			// FAIL
    ...
    ret = parsing_pattern_command_and_write_for_dp173(code_file_name,code_name);		// LWG : code_file_name(dp173_config.tty), code_name("LABEL_READ_PRE_WRITE"), write mipi
    if (ret < 0)
    {
        DERRPRINTF(" parsing_pattern_command_and_write_for_dp173\n");
        FUNC_END();
...
//        return ret;
	...
	ret = parsing_pattern_read_command(code_file_name,code_name,mipi_data);		// LWG : code_file_name(dp173_config.tty), code_name("LABEL_READ"), read mipi
    if (ret < 0)
    {
        DERRPRINTF(" parsing_pattern_read_command\n");
        FUNC_END();
        ...
        for (read_ch_cnt = 0;read_ch_cnt < read_ch_num;read_ch_cnt++)			// LWG : read_mipi 2 channels
        {
         ...
         //           return ret;
           ...
           //         return ret;
            ...
            usleep(3000);	/* 3ms delay */
            read_len = DP173_LABEL_MAX_LENGTH;
            ...
            if (ret < 0)
            {
             ...
             //   return ret;
			 	if(read_ch_cnt == 0)
					model_dp173_p->ch1_label_status = 2;			// FAIL
				else
					model_dp173_p->ch2_label_status = 2;			// FAIL
            }
			else
			{
				...
				read_data[4] = 0x37;
				*/
				for(i=0;i<DP173_LABEL_MAX_LENGTH;i++){

					printf("0x%02x ", read_data[i]);
				}
				printf("\n");
				printf("[dp173_config.tty] LABEL READ DATA ====== \n");
				for(i=0;i<DP173_LABEL_MAX_LENGTH;i++){
					printf("0x%02x ", read_buf[i]);
				}
				
				if(!strncmp(read_data, read_buf, DP173_LABEL_MAX_LENGTH))
				{   
					printf("LABEL PASS!!\n");
					pocb_status = pocb_status_p[read_ch_cnt][0] + 0 + 3;
					printf("%d                >>> LWG <<< [%s %d] %s CALL ====== \n", pocb_status, __FILE__, __LINE__, __FUNCTION__);
					if(read_ch_cnt == 0)
						model_dp173_p->ch1_label_status = 1;			// PASS
					else
						model_dp173_p->ch2_label_status = 1;			// PASS
				}
				else
				{   
					printf("LABEL FAIL!!\n");
...
/*
 * Name : pocb_write_for_dp173
 * Description : Write POCB data to DP173 to set POCB on/off.
 * Parameters :
 * 		unsigned char (*pocb_status_p)[] : write value of POCB status.
 * Return value : error
 */
int pocb_write_for_dp173(unsigned char (*pocb_status_p)[DP173_POCB_WRITE_LENGTH])
{
	int ret = 0;
	char code_file_name[MAX_FILE_NAME_LENGTH];
	char code_name[20];
	...
	write_ch_num = DP173_POCB_WRITE_CHANNEL_NUM;
...
/*
 * Name : init_pocb_status_for_dp173
 * Description : Set current POCB status to init POCB status.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void init_pocb_status_for_dp173(model_dp173_t *dp173_p)
{
	FUNC_BEGIN();

	/* set POCB current status to initial status */
	dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_1] = dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_1];
	dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_2] = dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_2];

	FUNC_END();
}

/*
 * Name : update_pocb_status_for_dp173
 * Description : Update(Write) POCB current status to display module.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void update_pocb_status_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	unsigned char pocb_write[DP173_POCB_WRITE_CHANNEL_NUM][DP173_POCB_WRITE_LENGTH];

	FUNC_BEGIN();

	/* write initial POCB status */
	memset(pocb_write,0,sizeof(pocb_write));
	pocb_write[POCB_CHANNEL_1][0] = dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_1];
	pocb_write[POCB_CHANNEL_2][0] = dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_2];
	ret = pocb_write_for_dp173(pocb_write);
	if (ret < 0)
	{
...
int reset_display_mode_for_dp173(model_dp173_t *dp173_p)
{
	...
	if (dp173_p->special_pattern_mode.pattern_mode != NORMAL_MODE)
	{
		/* control before displaying next image - mainly pattern off code or finishing thread */
		type_flag = SPECIAL_PATTERN_PREVIOUS_MODE;
		ret = control_special_pattern_mode_for_dp173(dp173_p,type_flag,code_file_name,dp173_p->special_pattern_mode.pattern_mode);
		if (ret < 0)
		{
		...
		dp173_p->special_pattern_mode.pattern_mode = NORMAL_MODE;
	}
...
/*
 * Name : display_current_test_image_for_dp173
 ...
 * Return value : 
 */
void display_current_test_image_for_dp173(model_dp173_t *dp173_p, int pattern_num)
{
	char comm[100] ={0,};
	...
	sprintf(current_test_dir,"%c/%s",dp173_info_p->dp173_manager.dir,CURRENT_DIR_NAME);
...
#if 0	
	/* DP173 test2 mode is AOD */
	if(pattern_num == 1)
		aod_on_by_command_for_dp173();
#endif

#if 1      
    /* 190108 DP173 current test1 mode is SCREEN MODE*/      
    if(pattern_num == 0){     
        system("/Data/reg_init /mnt/sd/initial/dp173_screen_on.tty");     
        //sleep(3);     
        DPRINTF("command : %s \n", comm);     // need for first pattern display
        system(comm);     
    }else{     
        system("/Data/reg_init /mnt/sd/initial/dp173_screen_off.tty");     
        DPRINTF("command : %s \n", comm);     
        system(comm);     
    }     
#endif 
...
// COPIED FOR get_measured_current_for_dp173
...
/*
 * Name : get_measured_current_for_dp173
 * Description : "DP173 SMILE VDDD & VDDI overcurrent issue", before measure VDDD, INA219 poweroff, VDDD INA219 bus voltage continuous
 * Parameters :
 *		int i2c_fd : i2c file descriptor.
 * Return value :
 * 		unsigned long : return measured current value.
 */
unsigned long get_measured_current_for_dp173(int i2c_fd)
{
	unsigned int half = 0;
	int i;
	short vl = 0;
	...
	//
	// DELETE MODE codes
	// mode will be set outside of this function ( get_current_test_result_for_dp173 )
	//
	//
...
/*
 * Name : get_measured_current_uA_for_dp173
 * Description : "DP173 SMILE VDDD & VDDI overcurrent issue", before measure VDDD, INA219 poweroff, VDDD INA219 bus voltage continuous
 * Parameters :
 *		int i2c_fd : i2c file descriptor.
 * Return value :
 * 		unsigned long : return measured current value.
 */
int get_measured_current_uA_for_dp173(int index, int uA_en, int ch)
{
...
/*
 * Name : get_current_test_result_for_dp173
 ...
 * Return value : error
 */
int get_current_test_result_for_dp173(model_dp173_t *dp173_p, int pattern_num, current_test_result_t (*result_p)[MAX_VOLT_NUM_VDDD])
{
	int ret = 0;
	int ch_cnt = 0;
	int index = 0;
	...
	int ina_slv = 0;
	short lgd_offset = 0;
	current_test_result_t current_test_result[DP173_CURRENT_TEST_CHANNEL_NUM][MAX_VOLT_NUM_VDDD];
	MODEL_MANAGER *dp173_manager_p = &dp173_p->model_dp173_info.dp173_manager;
	struct current_limit *current_limit_p = &dp173_manager_p->limit.current;
...
//
	...
	display_current_test_image_for_dp173(dp173_p,pattern_num);
	
	/* measure and get CURRENT test result */
	for (ch_cnt = 0;ch_cnt < DP173_CURRENT_TEST_CHANNEL_NUM;ch_cnt++)
	{
		/* set i2c device node */
...
//			continue;
        ...
        for(index = VCC1; index < current_limit_p->volt_count; index++)
        {
			...
			else if(index == VCC2)	/* VDDI - 1.8V */
			{
				DPRINTF("> VCC2 ------------ \n");
				#if 0
                if(pattern_num == DP173_CURRENT_PATTERN_WHITE) 
					err_f = 0.7;
				else if(pattern_num == DP173_CURRENT_PATTERN_40)
					err_f = 0.7;
				else if(pattern_num == DP173_CURRENT_PATTERN_SMILE)
					err_f = 0.72;		// 191121 ( 20% 더 감소 )
				else if(pattern_num == DP173_CURRENT_PATTERN_SLEEP)
					err_f = 0.7;
                #endif
                err_f = 1;
				temp_f = 0;
			...
			else if(index == TTL) /* VDDD */
			{
				...
				if(pattern_num == DP173_CURRENT_PATTERN_SMILE)
					err_f = 0.83;
#endif
				temp_f = 0;
			}
...
#if 0
			if((index == TTL) && (pattern_num == DP173_CURRENT_PATTERN_SMILE)){
				/* 2. VDDD START */
...
//			}
			...
			switch(index){
				case VCC1:		
					err_f = 1;
					if(pattern_num == DP173_CURRENT_PATTERN_SLEEP)
					{
						ucurrent_en(0);//uA		( DEEP SLEEP VPNL : 5uA ~ 10uA )
						// DEEP SLEEP 
						printf("execute deep sleep code\n");
						//system("/Data/reg_init /mnt/sd/initial/dp173_deep_sleep.tty 2>&1");		// silence
						system("/Data/reg_init /mnt/sd/initial/dp173_deep_sleep.tty > /dev/null");		// silence
                        usleep(1000);
						value = get_measured_current_uA_for_dp173(VCC1,1,ch_cnt);
					}
					else
					{
						ucurrent_en(1);//mA		( SLEEP VPNL : 5uA ~ 10uA )
						value = get_measured_current_uA_for_dp173(VCC1,0,ch_cnt);
					}
					break;
				...
				case VCC2:		
					...
					if(pattern_num == DP173_CURRENT_PATTERN_SLEEP)
					{
						...
						value = get_measured_current_uA_for_dp173(VCC2,1,ch_cnt);
					}
					else
					{
						ucurrent_en(1);//mA		( SLEEP VDDI : 500mA ~ 1000mA )
						value = get_measured_current_uA_for_dp173(VCC2,0,ch_cnt);
					}
					break;
				...
				case VDDVDH:
                    ...
                    {
                        ucurrent_en(1);//mA  
                        value = get_measured_current_for_dp173(i2c_fd);     // only for DP173
                    }
...
//			printf("err_f is %f\n", err_f);			// DEBUG ONLY
			...
			if (pattern_num == DP173_CURRENT_PATTERN_WHITE)
			{
				if (index == VCC1)	/* VPNL */
				{
					lgd_offset = DP173_CURRENT_WHITE_VPNL_LGD_OFFSET;
				}
				else if (index == VCC2)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_WHITE_VDDI_LGD_OFFSET;
				}
				else if (index == VDDVDH)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_WHITE_VDDVDH_LGD_OFFSET;
				}
				else if (index == VDDEL)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_WHITE_VDDEL_LGD_OFFSET;
				}
			}
			...
			else if (pattern_num == DP173_CURRENT_PATTERN_40)
			{
				if (index == VCC1)	/* VPNL */
				{
					lgd_offset = DP173_CURRENT_40_VPNL_LGD_OFFSET;
				}
				else if (index == VCC2)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_40_VDDI_LGD_OFFSET;
				}
				else if (index == VDDVDH)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_40_VDDVDH_LGD_OFFSET;
				}
				else if (index == VDDEL)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_40_VDDEL_LGD_OFFSET;
				}
			}
#endif	/* swchoi - end */
			else if (pattern_num == DP173_CURRENT_PATTERN_SMILE)
			{
				if (index == VCC1)	/* VPNL */
				{
					lgd_offset = DP173_CURRENT_SMILE_VPNL_LGD_OFFSET;
				}
				else if (index == VCC2)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SMILE_VDDI_LGD_OFFSET;
				}
				else if (index == VDDVDH)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SMILE_VDDVDH_LGD_OFFSET;
				}
				else if (index == VDDEL)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SMILE_VDDEL_LGD_OFFSET;
				}
#if 1
				else if (index == TTL)
				{
					lgd_offset = DP173_CURRENT_SMILE_VDDD_LGD_OFFSET;
				}
#endif
			}
			else if (pattern_num == DP173_CURRENT_PATTERN_SLEEP)
			{
				if (index == VCC1)	/* VPNL */
				{
					lgd_offset = DP173_CURRENT_SLEEP_VPNL_LGD_OFFSET;
				}
				else if (index == VCC2)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SLEEP_VDDI_LGD_OFFSET;
				}
				else if (index == VDDVDH)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SLEEP_VDDVDH_LGD_OFFSET;
				}
				else if (index == VDDEL)	/* VDDI */
				{
					lgd_offset = DP173_CURRENT_SLEEP_VDDEL_LGD_OFFSET;
				}
			}
...
//			lgd_offset = 0;//not used offset
            ...
            //if (current < 0)
			if ((current < 10) && ((pattern_num == DP173_CURRENT_PATTERN_40) && (index == VDDEL)))//1.0mA under to ALL zero
				current = 0;
		    ...
		    {
				if((pattern_num == DP173_CURRENT_PATTERN_SLEEP) && (index == VCC1 || index == VCC2))
...
int get_touch_test_result_for_dp173(int ch_num, unsigned int *test_result_p, model_dp173_t* dp173_p)
{
	int ret = 0;
	int err_ret = 0;

	FUNC_BEGIN();
	ret = atmel_init_i2c_set_slvAddr_depending_channel2_for_dp173(ch_num, 40, test_result_p);
	if (ret <= 0)
	{
...
//		DERRPRINTF("SPI NOT CONNECTED. TEST ALL FAIL.\n");
	...
	ret = FW_Ver_check_for_dp173(dp173_p);	// if value is 0.0.04, touch ic is solomon(1), if not, touch ic is synaptics(2).
	if (ret <= 0)
	{
	...
	if(dp173_p->touch_ic_kind == SOLOMON){
		...
		ret = micro_defect_check_for_dp173();
		if (ret <= 0)
		{
...
//		DTPRINTF("READ NEW LIMIT SPEC : DP173_SYNAP\n");
		synaptics_04_touch_limit_table_parser(Dp173.id, "DP173_SYNAP", Dp173.limit.ptouch);
		synaptics_04_init_limit_data(Dp173.limit.ptouch);

#if 1
//		DTPRINTF("DISPLAY TOUCH PATTERN : WHITE PATTERN\n");
		display_on_by_command_for_dp173();
    	{
//  	    printf("[WHITE] display white pattern\n");
		...
		ret = synaptics_04_current_avdd_dvdd_check(ch_num);		// 채널 번호 전달
		if (ret <= 0)
		{
...
// 작업완료
		...
		ret = synaptics_04_attention_check();
		if (ret <= 0)
		{
...
// 작업완료
		...
		ret = synaptics_04_fw_version_check();
		if (ret <= 0)
		{
		...
		ret = synaptics_04_touch_ic_config_check();
		if (ret <= 0)
		{
        ...
        ret = synaptics_04_device_package_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 작업완료, 스펙필요
        ...
        ret = synaptics_04_lockdown_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 패스처리
        ...
        ret = synaptics_04_attention2_check(ch_num);  		// GPIO CONTROL 필요 (새 함수 만들것)                                                                                                                 
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
        ...
        ret = synaptics_04_hsync_vsync_check();  		
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 테스트항목에서 제외됨
        ...
        ret = synaptics_04_bsc_calibration_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
		...
		ret = synaptics_04_reset_pin_check();
		if (ret <= 0)
		{
...
// 작업완료 
        ...
        ret = synaptics_04_sensor_speed_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 추가 라이브러리 이용, 마지막 행 전부 0으로 나오는건 아직 미해결
        ...
        ret = synaptics_04_extended_high_resist_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 작업완료
        ...
        ret = synaptics_04_adc_range_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {                                                                                                                                                                  
...
// 작업완료 
        ...
        ret = synaptics_04_full_raw_cap_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {
        ...
        ret = synaptics_04_full_raw_cap_v_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {
        ...
        ret = synaptics_04_full_raw_cap_h_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {
...
// 작업완료
        ...
        ret = synaptics_04_hybrid_abs_raw_cap_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {
...
// 작업완료
        ...
        ret = synaptics_04_customer_id_check();                                                                                                                      
        if (ret <= 0)                                                                                                                                                      
        {
...
/*
 * Name : otp_key_action_for_dp173
 * Description : Key action for OTP key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void otp_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	unsigned char otp_value[DP173_OTP_READ_CHANNEL_NUM][DP173_OTP_READ_LENGTH];
	unsigned char pocb_status[DP173_POCB_READ_CHANNEL_NUM][DP173_POCB_READ_LENGTH];
	unsigned char additional_info = 0;
	unsigned char otp_write_num[DP173_OTP_READ_CHANNEL_NUM] = {0,0};
	int ch_cnt = 0;
	int debug_ch_cnt = 0;
	int debug_data_cnt = 0;

	unsigned char otp_connect_flag[DP173_OTP_READ_CHANNEL_NUM] = {0,0};

	FUNC_BEGIN();

	if (dp173_p->cur_test_mode == VFOS_DISPLAY_TEST_MODE)
	{
		ret = reset_display_mode_for_dp173(dp173_p);
		if (ret < 0)
		{
			DERRPRINTF("reset_display_mode_for_dp173\n");
		}
	}

	if (dp173_p->flag_need_to_init_module == true)
	{
		display_module_on_for_dp173(model_dp173_p,model_index);
		display_black_pattern_for_vfos();
	}
	...
	ret = dbv_read_for_dp173(otp_value);
	if (ret < 0)
	{
		DERRPRINTF("dbv_read_for_dp173\n");
	}
	else
	{
		/* debug print to check DBV read value */
		DPRINTF("DBV_READ: ");
		for (debug_ch_cnt = 0;debug_ch_cnt < DP173_OTP_READ_CHANNEL_NUM;debug_ch_cnt++)
		{
			for (debug_data_cnt = 0;debug_data_cnt < DP173_OTP_READ_LENGTH;debug_data_cnt++)
			{
	...
	/* get OTP value */
	memset(otp_value,0,sizeof(otp_value));
	ret = otp_read_for_dp173(otp_value);
	if (ret < 0)
	{
		DERRPRINTF("otp_read_for_dp173\n");
	}
	else
	{
	...
	// if PCD CONNECT FAIL --> send NC to UI
	if(otp_connect_flag[0] == 0){
		otp_value[0][0] = 0xFE;
	}
	if(otp_connect_flag[1] == 0){
		otp_value[1][0] = 0xFE;
	}

//	int temp = 0;
	unsigned char temp[DP173_POCB_READ_CHANNEL_NUM][DP173_POCB_READ_LENGTH];

	/* get POCB value and check status */
	ret = pocb_read_for_dp173(pocb_status);
	if (ret < 0)
	{
		DERRPRINTF("pocb_read_for_dp173\n");
	}
	else
	{
		DPRINTF("POCB_STATUS: CH1=[%d],CH2=[%d]\n",pocb_status[POCB_CHANNEL_1][0],pocb_status[POCB_CHANNEL_2][0]);
		if (dp173_p->pocb_status.flag_pocb_changed == POCB_STATUS_NO_CHANGE)
		{
			for (ch_cnt = 0;ch_cnt < DP173_POCB_READ_CHANNEL_NUM;ch_cnt++)
			{
				temp[ch_cnt][0] = pocb_status[ch_cnt][0];
				pocb_status[ch_cnt][0] = 2;
				dp173_p->pocb_status.pocb_init_status[ch_cnt] = pocb_status[ch_cnt][0];	/* set init status */
			}
		}
	}

	/* send OTP value and POCB status to UI by UART */
	for (ch_cnt = 0;ch_cnt < DP173_OTP_READ_CHANNEL_NUM;ch_cnt++)
	{
//		send_otp_key_to_uart(ch_cnt + 1,otp_write_num[ch_cnt],DP173_OTP_MAX_WRITE_TIME,pocb_status[ch_cnt][0],additional_info);
		send_otp_key_to_uart(ch_cnt + 1,otp_value[ch_cnt][0],DP173_OTP_MAX_WRITE_TIME, temp[ch_cnt][0] ,additional_info);
	}


	// LWG 191101 DP173 PCD USE OTP

	printf("pcd_value : %d %d\n", otp_value[0][0], otp_value[1][0]);
	dp173_p -> flag_otp_test_result_ch1 = 0;		// 재 테스트를 위한 초기화	
	dp173_p -> flag_otp_test_result_ch2 = 0;		// 재 테스트를 위한 초기화	
	for (ch_cnt = 0;ch_cnt < DP173_OTP_READ_CHANNEL_NUM;ch_cnt++)
	{
		switch(otp_value[ch_cnt][0]){
			case 0:
				if(ch_cnt == 0){
					dp173_p -> flag_otp_test_result_ch1 = 1;		// OTP TEST PASS
				}else{
					dp173_p -> flag_otp_test_result_ch2 = 1;		// OTP TEST PASS
				}
				break;
			default:
				if(ch_cnt == 0){
					dp173_p -> flag_otp_test_result_ch1 = 2;		// OTP TEST FAIL
				}else{
					dp173_p -> flag_otp_test_result_ch2 = 2;		// OTP TEST FAIL
				}
				break;
		}
	}
	...
	if(dp173_p->ch1_pocb_status == 2){
		dp173_p -> flag_otp_test_result_ch1 = 2;        // OTP TEST FAIL
	}

	if(dp173_p->ch2_pocb_status == 2){
		dp173_p -> flag_otp_test_result_ch2 = 2;        // OTP TEST FAIL
	}

	/* CHECK FOR LABEL, if LABEL FAIL, INTERLOCK applies */
	if(dp173_p->ch1_label_status == 2){
		dp173_p -> flag_otp_test_result_ch1 = 2;        // OTP TEST FAIL
	}

	if(dp173_p->ch2_label_status == 2){
		dp173_p -> flag_otp_test_result_ch2 = 2;        // OTP TEST FAIL
	}

	display_module_off_for_dp173(dp173_p,model_index);
	FUNC_END();
}

/*
 * Name : touch_key_action_for_dp173
 * Description : Key action for TOUCH key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void touch_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	...
	touch_limit_p = (struct atmel_03_touch_limit *)dp173_manager_p->limit.ptouch;
	...
	//send_function_start_info_to_uart(TOUCH);	// DP173 은 바로 TOUCH UI를 실행시키지 않고, TOUCH IC 종류를 파악하고 한다.

	/* init touch test */
	init_tch_power_set_for_dp173(1);	// [LWG] TOUCH POWER IS ON
	
	printf("------------------------------------------------------------ \n");
	
	atmel_03_init_limit_data(touch_limit_p);
	dp173_p -> flag_touch_test_result_ch1 = 0;
	dp173_p -> flag_touch_test_result_ch2 = 0;
	/* run touch test and send test result to UI */
	for(ch_cnt = 1; ch_cnt <3; ch_cnt++)
	{
		result = 0x0;	/* init test result - swchoi */
		//if(ch_cnt == 1)		// LWG
		ret = get_touch_test_result_for_dp173(ch_cnt,&result,&dp173_p);	// dp173 touch info is in dp173_p.
		if (ret < 0)
		{
			DERRPRINTF("get_touch_test_result_for_dp173(ch_num=(%d),test_result=0x%08x)\n",ch_cnt,result);
		}
		else
		{
		...
		hf_test_on = false;	/* hf_test is used for only JOAN model, so hf_test_on has to be false for DP173 */
		...
		if(result){		// 하나라도 1이 있으면 fail
				if(ch_cnt == 1){
						dp173_p -> flag_touch_test_result_ch1 = 2;		// FAIL
				}else{
						dp173_p -> flag_touch_test_result_ch2 = 2;		// FAIL
				}
		//else if((dp173_p -> flag_touch_test_result) != 2){
		}else if(!result){
				if(ch_cnt == 1){
						if(dp173_p -> flag_touch_test_result_ch1 != 2){
								dp173_p -> flag_touch_test_result_ch1 = 1;		// PASS
						}
				}else{
						if(dp173_p -> flag_touch_test_result_ch2 != 2){
								dp173_p -> flag_touch_test_result_ch2 = 1;		// PASS
						}
				}
		}
	}
	if((model_dp173_p -> flag_synaptics) == 1){		// 모델 원복
		send_toggle_synaptics_to_uart();
		(model_dp173_p -> flag_synaptics) = 0;
	}
	usleep(30000);
	/* power off */
	init_tch_power_set_for_dp173(0);
...
/*
 * Name : current_key_action_for_dp173
 * Description : Key action for CURRENT key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void current_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int pattern_cnt = 0;
	int ch_cnt = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	struct current_limit *current_limit_p = &dp173_p->model_dp173_info.dp173_manager.limit.current;
	...
	dp173_p -> flag_current_test_result_ch1 = 0;
	dp173_p -> flag_current_test_result_ch2 = 0;
	...
	for (pattern_cnt = 0;pattern_cnt < current_limit_p->pattern_count;pattern_cnt++)
	{
		...
		if(pattern_cnt == DP173_CURRENT_PATTERN_SLEEP){
			int ret;
			...
			memcpy(&(dp173_p -> special_pattern_mode), &mode, sizeof(special_pattern_mode_t));
			DPRINTF("SLEEP FILE\n");

			ret = set_display_mode_for_dp173(dp173_p);
			if (ret < 0)
			{
				DERRPRINTF("set_display_mode_for_dp173\n");
			}
		}
		
		/* get current test result such as voltage and current */
		ret = get_current_test_result_for_dp173(dp173_p,pattern_cnt,dp173_p->current_test_result);
		...
		if (ret < 0)
		{
			DERRPRINTF("get_current_test_result_for_dp173\n");
		}

		for (ch_cnt = 0;ch_cnt < DP173_CURRENT_TEST_CHANNEL_NUM;ch_cnt++)
		{
			send_current_test_result_to_uart_for_dp173(dp173_p->current_test_result,current_limit_p->pattern_count,pattern_cnt,ch_cnt);
		}

		// judge_interlock_current 시작 : '각 이미지' 별로 is_over_limit 가 누적되어어야 한다
		judge_interlock_current_for_dp173(dp173_p);
...
/*
 * Name : func_key_action_for_dp173
 * Description : Key action for FUNC key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void func_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	...
	if (dp173_p->flag_already_module_on == true)
	{
		display_module_off_for_dp173(dp173_p,model_index);
...
/*
 * Name : next_key_action_for_dp173
 * Description : Key action for NEXT(TURN) key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void next_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	model_dp173_info_t *dp173_info_p = &dp173_p->model_dp173_info;
	MODEL_MANAGER *manager_p = &dp173_info_p->dp173_manager;
	int image_count = manager_p->limit.display.image_count;
	char image_file_name[MAX_FILE_NAME_LENGTH];
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	char pattern_num_str[MAX_PATTERN_NUM_STRING_NUM] = {0,};

	unsigned long long temp_pattern_mode = dp173_p->special_pattern_mode.pattern_mode;
	...
	dp173_p->cur_image_num = (dp173_p->cur_image_num + 1) % (image_count + 1);
	if (dp173_p->cur_image_num <= 0)
	{
		dp173_p->cur_image_num = 1;
	}

	/* parsing pattern mode from image file name */
	strcpy(&image_file_name[0],&dp173_info_p->display_image_file_name[dp173_p->cur_image_num - 1][0]);
	DPRINTF("cur_image_num=(%d),image_file_name=(%s)\n",dp173_p->cur_image_num,&image_file_name[0]);


	//braightosurojce
	parsing_pattern_start_end_mode(&image_file_name[0],&dp173_p->special_pattern_mode);
	if((dp173_p->special_pattern_mode.pattern_mode & START_MODE) == START_MODE)
	{
		dp173_p->special_pattern_mode.pattern_mode = temp_pattern_mode;
		ret = reset_display_mode_for_dp173(dp173_p);
		if (ret < 0)
		{
			DERRPRINTF("reset_display_mode_for_dp173\n");
	...
	else if((dp173_p->special_pattern_mode.pattern_mode & END_MODE) == END_MODE)
	{
	...
	ret = parsing_pattern_mode(&image_file_name[0],&dp173_p->special_pattern_mode);
	if (ret < 0)
	{
	...
	{
		DPRINTF("Pattern mode=(0x%x)\n", dp173_p->special_pattern_mode.pattern_mode);

		/* display white pattern for only dimming thread */
		if (((dp173_p->special_pattern_mode.pattern_mode & DIMMING_MODE) == DIMMING_MODE) && ((dp173_p->special_pattern_mode.pattern_mode & DBV_MODE) == DBV_MODE))
		{
		...
		if(((dp173_p->special_pattern_mode.pattern_mode & AOD_MODE) == AOD_MODE) || ((dp173_p->special_pattern_mode.pattern_mode & VOLTAGE_MODE) == VOLTAGE_MODE)){
			printf("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n\n");
			display_image_for_vfos(dp173_info_p->display_image_dir,image_file_name);
		}
#endif

        //brightsouce 191226
		//feather973  200109
		if((dp173_p->special_pattern_mode.pattern_mode & AOD_MODE) == AOD_MODE)
        {
            display_image_for_vfos(dp173_info_p->display_image_dir,image_file_name);
        }
		else if((dp173_p->special_pattern_mode.pattern_mode & DBV_5RD_NIT_MODE) == DBV_5RD_NIT_MODE)
		{
			display_image_for_vfos(dp173_info_p->display_image_dir,image_file_name);
		}
		else if((dp173_p->special_pattern_mode.pattern_mode & BLACKPOINT_MODE) == BLACKPOINT_MODE)
		{
			display_image_for_vfos(dp173_info_p->display_image_dir,image_file_name);
		}
	...
	if ((dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_1] != POCB_STATUS_NO_READ) && (dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_2] != POCB_STATUS_NO_READ))
	{
		/* initialize POCB status */
		init_pocb_status_for_dp173(dp173_p);
		if ((dp173_p->special_pattern_mode.pattern_mode & POCB_WRITE_ENABLE_MODE) == POCB_WRITE_ENABLE_MODE)
		{
			/* write POCB status to display module */
			update_pocb_status_for_dp173(dp173_p);
		}
	}

	/* display image - except gray scan and dimming and gradation */
	if (((dp173_p->special_pattern_mode.pattern_mode & GRAY_MODE) != GRAY_MODE) && (((dp173_p->special_pattern_mode.pattern_mode & DIMMING_MODE) != DIMMING_MODE) && ((dp173_p->special_pattern_mode.pattern_mode & DBV_MODE) != DBV_MODE)) && ((dp173_p->special_pattern_mode.pattern_mode & GRAD_MODE) != GRAD_MODE))
	{
		display_image_for_vfos(dp173_info_p->display_image_dir,image_file_name);
	}
	else if ((dp173_p->special_pattern_mode.pattern_mode & GRAD_MODE) == GRAD_MODE)	/* display pattern for Gradation display */
	{
		if ((dp173_p->special_pattern_mode.pattern_mode & GRAD_RGB_MODE) == GRAD_RGB_MODE)
		{
			if ((dp173_p->special_pattern_mode.pattern_mode & HORIZONTAL_TOP_MODE) == HORIZONTAL_TOP_MODE)
			{
				strcpy(pattern_num_str,GRAD_RGB_HT_PATTERN_COMMAND_NUM_STRING);
				display_pattern_for_vfos(pattern_num_str);
			}
			else if ((dp173_p->special_pattern_mode.pattern_mode & HORIZONTAL_DOWN_MODE) == HORIZONTAL_DOWN_MODE)
			{
				strcpy(pattern_num_str,GRAD_RGB_HD_PATTERN_COMMAND_NUM_STRING);
				display_pattern_for_vfos(pattern_num_str);
			}
			else if ((dp173_p->special_pattern_mode.pattern_mode & VERTICAL_LEFT_MODE) == VERTICAL_LEFT_MODE)
			{
				strcpy(pattern_num_str,GRAD_RGB_VL_PATTERN_COMMAND_NUM_STRING);
				display_pattern_for_vfos(pattern_num_str);
			}
			else if ((dp173_p->special_pattern_mode.pattern_mode & VERTICAL_RIGHT_MODE) == VERTICAL_RIGHT_MODE)
			{
				strcpy(pattern_num_str,GRAD_RGB_VR_PATTERN_COMMAND_NUM_STRING);
				display_pattern_for_vfos(pattern_num_str);
			}
		}
		else if ((dp173_p->special_pattern_mode.pattern_mode & GRAD_RED_MODE) == GRAD_RED_MODE)
		{
			if ((dp173_p->special_pattern_mode.pattern_mode & HORIZONTAL_TOP_MODE) == HORIZONTAL_TOP_MODE)
			{
	...
	if (dp173_p->flag_need_to_display_on == true)
	{
		usleep(100000);	/* 100ms delay between displaying image and display on command as LGD provided */
		display_on_by_command_for_dp173();
		dp173_p->flag_need_to_display_on = false;
	}

	/* FOR CRC READ */
	printf("[LWG] image_file_name is %s\n", image_file_name);
	if (strstr(image_file_name, "checksum") != NULL){
		unsigned char crc_write_value[DP173_CRC_READ_CHANNEL_NUM][DP173_CRC_READ_LENGTH];
		unsigned char crc_read_value[DP173_CRC_READ_CHANNEL_NUM][DP173_CRC_READ_LENGTH];
		unsigned char crc_write_num[DP173_CRC_READ_CHANNEL_NUM] = {0,0};
		...
		ret = crc_read_for_dp173(crc_write_value, crc_read_value);
		if (ret < 0)
		{
				DERRPRINTF("crc_read_for_dp173\n");
		}
		...
		else
		{
				...
				for (debug_ch_cnt = 0;debug_ch_cnt < DP173_CRC_READ_CHANNEL_NUM;debug_ch_cnt++)
				{
						for (debug_data_cnt = 0;debug_data_cnt < DP173_CRC_READ_LENGTH;debug_data_cnt++)
						{
		...
		if(!strcmp(image_file_name, "46_gram_checksum_1.bmp")){		// checksum_1 이미지에 대해 판정시작
			...
			judge = 0;
#if 0	
			unsigned char lgd_crc_spec[DP173_CRC_READ_LENGTH] = { 0xDA, 0x2F, 0xA2, 0x35 };				// LGD CRC스펙
			unsigned char hybus_crc_spec[DP173_CRC_READ_LENGTH] = { 0xDA, 0x2F, 0xA2, 0x35 };			// HYBUS CRC스펙
...
//				printf("[0x%02x] ", lgd_crc_spec[i]);
			...
			judge = 0;
#if 0
			unsigned char lgd_crc_spec[DP173_CRC_READ_LENGTH] = { 0xF0, 0xF4, 0x14, 0xD6 };					// LGD CRC스펙
			unsigned char hybus_crc_spec[DP173_CRC_READ_LENGTH] = { 0xF0, 0xF4, 0x14, 0xD6 };				// 우리 CRC스펙
...
/*
 * Name : prev_key_action_for_dp173
 * Description : Key action for PREV(RETURN) key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void prev_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	model_dp173_info_t *dp173_info_p = &dp173_p->model_dp173_info;
	MODEL_MANAGER *manager_p = &dp173_info_p->dp173_manager;
	...
	/* change current image number */
	dp173_p->cur_image_num--;
	if (dp173_p->cur_image_num <= 0)
	{
		dp173_p->cur_image_num = image_count;
	}
...
/*
 * Name : reset_key_action_for_dp173
 * Description : Key action for RESET key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void reset_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
...
/*
 * Name : func2_key_action_for_dp173
 * Description : Key action for FUNC2(SET) key input.
 * Parameters :
 * 		model_dp173_t *dp173_p
 * Return value : 
 */
void func2_key_action_for_dp173(model_dp173_t *dp173_p)
{
	int ret = 0;
	int ch_cnt = 0;
	int model_index = dp173_p->model_dp173_info.buf_index + 1;
	unsigned char pocb_cur_status[DP173_POCB_WRITE_CHANNEL_NUM];
	unsigned char pocb_write[DP173_POCB_WRITE_CHANNEL_NUM][DP173_POCB_WRITE_LENGTH];
	...
	if (dp173_p->flag_need_to_pocb_write == true)
	{
		memcpy(pocb_cur_status,dp173_p->pocb_status.pocb_cur_status,sizeof(pocb_cur_status));
		for (ch_cnt = 0;ch_cnt < DP173_POCB_WRITE_CHANNEL_NUM;ch_cnt++)
		{
			...
			dp173_p->pocb_status.pocb_cur_status[ch_cnt] = pocb_cur_status[ch_cnt];
		...
		printf("[LWG] POCB WRITE FOR DP173]\n");
		ret = pocb_write_for_dp173(pocb_write);
		if (ret < 0)
		{
			DERRPRINTF("pocb_write\n");
		...
		if (dp173_p->pocb_status.flag_pocb_changed == POCB_STATUS_NO_CHANGE)
		{
			dp173_p->pocb_status.flag_pocb_changed = POCB_STATUS_CHANGED;
		}
	}
...
/*
 * Name : key_action_for_dp173
 * Description : Key action for each key input.
 * Parameters :
 * 		int key_value : input key value.
 * Return value : whether or not exit of thread
 */
int key_action_for_dp173(int key_value)
{
	int is_exit = 0;
	int pocb_write_enable = 0;
	model_dp173_t *dp173_p = model_dp173_p;
	model_dp173_info_t *dp173_info_p = &dp173_p->model_dp173_info;

	FUNC_BEGIN();

	DPRINTF("#######DP173_THREAD:key (%d) is pushed#########\n", key_value);
#if 0
	if(flag_judge == 1){
		is_exit = false;
		dp173_p->last_key_value = key_value;
		return is_exit;
	}else 
#endif
	...
	if (key_value == OTP)
	{
//		if ((dp173_p -> flag_otp_test_result == 1) && (flag_interlock == 1))	return is_exit;		// 성공한 테스트는 재수행하지 않는다.  

		dp173_p->flag_need_to_init_module = true;	/* always initialize display module */

		otp_key_action_for_dp173(dp173_p);

		dp173_p->cur_test_mode = VFOS_OTP_TEST_MODE;
		is_exit = false;
		dp173_p->last_key_value = key_value;
	}
	else if (key_value == TOUCH)
	{
//		if ((dp173_p -> flag_touch_test_result == 1) && (flag_interlock == 1))    return is_exit;     // 성공한 테스트는 재수행하지 않는다.

		dp173_p->flag_need_to_init_module = true;	/* always initialize display module */

		touch_key_action_for_dp173(dp173_p);

		dp173_p->cur_test_mode = VFOS_TOUCH_TEST_MODE;
		is_exit = false;
		dp173_p->last_key_value = key_value;
	}
	else if (key_value == CURRENT)
	{
//		if ((dp173_p -> flag_current_test_result == 1) && (flag_interlock == 1))    return is_exit;     // 성공한 테스트는 재수행하지 않는다.

		dp173_p->flag_need_to_init_module = true;	/* always initialize display module */

		current_key_action_for_dp173(dp173_p);

		dp173_p->cur_test_mode = VFOS_CURRENT_TEST_MODE;
		is_exit = false;
		dp173_p->last_key_value = key_value;
	}
//	else if (key_value == FUNC)
	...
	{
		dp173_p->flag_need_to_init_module = false;
		func_key_action_for_dp173(dp173_p);
		DPRINTF("#######DP173_THREAD:EXIT as (%d) is pushed#########\n", key_value);
		is_exit = true;
		dp173_p->last_key_value = key_value;
	}
	else if (key_value == NEXT)
	{
		...
		if(((dp173_p -> flag_otp_test_result_ch1 != 1) || (dp173_p -> flag_otp_test_result_ch2 != 1) 
			|| (dp173_p -> flag_touch_test_result_ch1 != 1) || (dp173_p -> flag_touch_test_result_ch2 != 1)
			|| (dp173_p -> flag_current_test_result_ch1 != 1) || (dp173_p -> flag_current_test_result_ch2 != 1))
			&& (flag_interlock)){
				printf("\n LWG judge\n");
				printf("flag_current_test_result_ch1 is %d, flag_current_test_result_ch2 is %d\n", 
								dp173_p -> flag_current_test_result_ch1, dp173_p -> flag_current_test_result_ch2);
				send_judge_status_to_uart(dp173_p -> flag_otp_test_result_ch1, dp173_p -> flag_otp_test_result_ch2,
								dp173_p -> flag_touch_test_result_ch1, dp173_p -> flag_touch_test_result_ch2,
								dp173_p -> flag_current_test_result_ch1, dp173_p -> flag_current_test_result_ch2);
				flag_judge = 1;
				return is_exit;

		// 모든 테스트를 통과하고 NEXT 누르면(화상검사) judge 플래그를 끈다. (재 테스트시 사용됨)
		}else if(((dp173_p -> flag_otp_test_result_ch1 == 1) && (dp173_p -> flag_otp_test_result_ch2 == 1) 
				&& (dp173_p -> flag_touch_test_result_ch1 == 1) && (dp173_p -> flag_touch_test_result_ch2 == 1)
				&& (dp173_p -> flag_current_test_result_ch1 == 1) && (dp173_p -> flag_current_test_result_ch2 == 1))){
				printf("\n LWG all pass\n");			
				flag_judge = 0;
...
#endif
		if ((dp173_p->last_key_value == OTP) || (dp173_p->last_key_value == TOUCH) || (dp173_p->last_key_value == CURRENT)|| (dp173_p->last_key_value == FUNC)|| (dp173_p->last_key_value == RESET))
		{
			dp173_p->flag_need_to_init_module = true;
		}
		...
		next_key_action_for_dp173(dp173_p);

		dp173_p->cur_test_mode = VFOS_DISPLAY_TEST_MODE;
		is_exit = false;
		dp173_p->last_key_value = key_value;

		/* send display information to UI */
		send_display_info_to_uart_for_dp173(key_value,dp173_p);
		...
		send_pocb_status_to_uart(dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_1],dp173_p->pocb_status.pocb_cur_status[POCB_CHANNEL_2],dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_1],dp173_p->pocb_status.pocb_init_status[POCB_CHANNEL_2],pocb_write_enable);
	}
	else if (key_value == PREV)
	{
		...
		prev_key_action_for_dp173(dp173_p);
	...
	else if (key_value == RESET)
	{
		printf("LWG %d %d, (%d %d) (%d %d) (%d %d)\n", 
						...
						dp173_p -> flag_otp_test_result_ch1, dp173_p -> flag_otp_test_result_ch2,
						dp173_p -> flag_touch_test_result_ch1, dp173_p -> flag_touch_test_result_ch2, 
						dp173_p -> flag_current_test_result_ch1, dp173_p -> flag_current_test_result_ch2);
		// LWG 190327
		if(flag_judge == 1){
			goto RESET;
		}else if((dp173_p->flag_need_to_init_module == false) && (dp173_p->flag_already_module_on == false) && (is_display_on_for_next_prev == 0)){ 
//			printf("                >>> LWG <<< [%s %d] %s CALL ====== %d %d %d \n", __FILE__, __LINE__, __FUNCTION__,
//					(dp173_p->flag_need_to_init_module == false)?0:1, (dp173_p->flag_already_module_on == false)?0:1, is_display_on_for_next_prev);
			//printf("\n LWG interlock\n");
			//flag_interlock = ((dp173_p -> flag_display_test_result) == 0)?1:0;
			...
			printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
#if 0
		}else if(((dp173_p -> flag_otp_test_result != 1) || (dp173_p -> flag_touch_test_result != 1) ||
				(dp173_p -> flag_current_test_result != 1) || (dp173_p -> flag_display_test_result != 1))
						&& (flag_interlock)){
			printf("\n LWG judge\n");
			send_judge_status_to_uart(dp173_p -> flag_otp_test_result, dp173_p -> flag_touch_test_result,
			dp173_p -> flag_current_test_result,dp173_p -> flag_display_test_result);
			flag_judge = 1;
#endif
		}
		else{	
RESET:
		dp173_p -> flag_otp_test_result_ch1 = 0;
		dp173_p -> flag_otp_test_result_ch2 = 0;
		dp173_p -> flag_touch_test_result_ch1 = 0;
		dp173_p -> flag_touch_test_result_ch2 = 0;
		dp173_p -> flag_current_test_result_ch1 = 0;
		dp173_p -> flag_current_test_result_ch2 = 0;
		...
		/* reset key action */
		reset_key_action_for_dp173(dp173_p);

		dp173_p->cur_test_mode = VFOS_RESET_MODE;
		is_exit = false;
		dp173_p->last_key_value = key_value;

		send_reset_key_to_uart();
		send_func_key_to_uart(&dp173_info_p->version_info,dp173_info_p->model_dp173_id);

		//flag_interlock = 1;		//	RESET need to interlock flag

		/* Initialize variables to make init condition */
		init_variable_for_dp173(dp173_p);
...
//	else if (key_value == FUNC2)
	else if ((key_value == FUNC2) && flag_judge != 1)
	{
		...
		if ((dp173_p->last_key_value == OTP) || (dp173_p->last_key_value == TOUCH) || (dp173_p->last_key_value == CURRENT) || (dp173_p->last_key_value == NEXT) || (dp173_p->last_key_value == PREV))
		{
			dp173_p->flag_need_to_init_module = false;
		}
...
//		if ((dp173_p->last_key_value == NEXT) || (dp173_p->last_key_value == PREV))
		if (dp173_p->cur_test_mode == VFOS_DISPLAY_TEST_MODE)
		{
			if ((dp173_p->special_pattern_mode.pattern_mode & POCB_WRITE_ENABLE_MODE) == POCB_WRITE_ENABLE_MODE)
			{
				dp173_p->flag_need_to_pocb_write = true;
			}
			else
			{
				dp173_p->flag_need_to_pocb_write = false;
			}
		}
		...
		func2_key_action_for_dp173(dp173_p);
		/* send info to UI */
		if (dp173_p->flag_need_to_init_module == true)
		{
			send_vfos_touch_version_to_uart(&dp173_p->model_dp173_info.version_info,dp173_p->model_dp173_info.model_dp173_id);
			send_vfos_display_version_to_uart(&dp173_p->model_dp173_info.version_info,dp173_p->model_dp173_info.buf_index+1);
		}
		if (dp173_p->flag_need_to_pocb_write == true)
		{
			pocb_write_enable = true;
	...
	{
		DERRPRINTF("#######DP173_THREAD:Other key is pushed(key=%d)#########\n", key_value);
		is_exit = false;
		dp173_p->last_key_value = key_value;
	}
...
/*
 * Name : dp173_thread
 * Description : Thread for model DP173.
 * Parameters :
 * 		void *arg : arguments for dp173_thread.
 * Return value : NULL
 */
void *dp173_thread(void *arg)
{
	int is_exit = 0;
	int thread_loop = 0;
	int key_value = 0;
	...
	model_dp173_info_t *dp173_info_p = &dp173_p->model_dp173_info;
//	unsigned char uart_buf[MAX_PACKET];
	pthread_mutex_t	*mutex_p = &dp173_p->dp173_thread_mutex;

	FUNC_BEGIN();

	pthread_mutex_lock(mutex_p);
	get_info_for_dp173_thread();

	DPRINTF("######### Start DP173 thread ###########\n");
...
//goto DP173;
	...
	while (thread_loop)
	{
		key_value = read_key_input_for_dp173();
		dp173_p->cur_key_value = key_value;
		if (key_value > 0)
		{
//DP173:
//key_value = TOUCH;
			is_exit = key_action_for_dp173(key_value);
			if (is_exit == true)
			{
				thread_loop = false;
			}
	...
	send_func_key_to_uart(&dp173_info_p->version_info,dp173_info_p->next_model_id);
...
/*
 * Name : init_model_dp173
 * Description : Initialize for model DP173, called by main() function.
 * Parameters :
...
int init_model_dp173(void)
{
	...
	model_dp173_p = malloc(sizeof(model_dp173_t));
	if (model_dp173_p == NULL)
	{
		DERRPRINTF(" Allocate memory for model_dp173_p\n");
		FUNC_END();
		return -1;
	}

	/* initialize mutex */
	ret = pthread_mutex_init(&model_dp173_p->dp173_thread_mutex,NULL);
	if (ret < 0)
	{
		DERRPRINTF(" pthread_mutex_init(errno=%d)\n", errno);
		FUNC_END();
...
/*
 * Name : release_model_dp173
 * Description : Release for model DP173, called by main() function.
 * Parameters :
 * Return value : error code
 */
...
void start_dp173_thread(void)
{
	...
	ret = pthread_create(&model_dp173_p->id_dp173_thread, NULL, dp173_thread, (void *)(model_dp173_p));
	if (ret < 0)
	{
		DERRPRINTF(" pthread_create(errno=%d)\n", errno);
		FUNC_END();
	...
	{
		pthread_join(model_dp173_p->id_dp173_thread,NULL);
	}
...
/*
 * Name : get_last_key_value_for_dp173
...
int get_last_key_value_for_dp173(void)
{
	...
	pthread_mutex_t	*mutex_p = &model_dp173_p->dp173_thread_mutex;

	FUNC_BEGIN();

	pthread_mutex_lock(mutex_p);
	last_key_value = model_dp173_p->last_key_value;
	pthread_mutex_unlock(mutex_p);
...
void judge_interlock_current_for_dp173(model_dp173_t *dp173_p){ 
	int ch_cnt;
	for(ch_cnt=0;ch_cnt<DP173_CURRENT_TEST_CHANNEL_NUM;ch_cnt++){
		if (
			((dp173_p->current_test_result)[ch_cnt][VCC1].is_over_limit & 0xff) |
			((dp173_p->current_test_result)[ch_cnt][VCC2].is_over_limit & 0xff) |
			((dp173_p->current_test_result)[ch_cnt][VDDVDH].is_over_limit & 0xff) |
			((dp173_p->current_test_result)[ch_cnt][VDDEL].is_over_limit & 0xff) |
			((dp173_p->current_test_result)[ch_cnt][TTL].is_over_limit & 0xff)
		){
			printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
			if(ch_cnt == 0){
				printf("CH1 FAIL\n");
				dp173_p -> flag_current_test_result_ch1 = 2;		// FAIL
			}else{
				dp173_p -> flag_current_test_result_ch2 = 2;		// FAIL
			}
		//}else if((dp173_p -> flag_current_test_result) != 2){
		}else if(
			!((dp173_p->current_test_result)[ch_cnt][VCC1].is_over_limit & 0xff) &
			!((dp173_p->current_test_result)[ch_cnt][VCC2].is_over_limit & 0xff) &
			!((dp173_p->current_test_result)[ch_cnt][VDDVDH].is_over_limit & 0xff) &
			...
			printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
			if(ch_cnt == 0){
				if(dp173_p -> flag_current_test_result_ch1 != 2){
					printf("CH1 SUCCESS\n");
					dp173_p -> flag_current_test_result_ch1 = 1;		// SUCCESS
				}
			}else{
				if(dp173_p -> flag_current_test_result_ch2 != 2){
					dp173_p -> flag_current_test_result_ch2 = 1;		// SUCCESS
				}
			}
		}
	}
