...
void init_tch_power_set_for_dp116(int on)
{
	int power = 0;
...
u8 p_for_dp116[120] = {0,};
u8 *ID_Info_for_dp116=p_for_dp116;

u8 mem_for_dp116[sizeof(u8)*130*11+1] = {0,};//130(64+64+2) * 11page + 1
u8 *read_buf_global_for_dp116 = mem_for_dp116;

struct atmel_03_touch_limit l_limit;
int mxt_write_blks_for_dp116(u16 start, u16 count, u8 *buf);
...
static int mxt_read_page(u16 start, u16 count, u8 *buf)
{
	...
	while (offset_for_count < count-2)
	{
		...
		//page_up
		write_cmd = 0x01;
		mxt_write_blks_for_dp116(touch_table.t6_cfg_address+5, 1, &write_cmd);
	}
	return ret_val;
}
...
static int mxt_parse_object_table(struct mxt_data *mxtdata, struct mxt_object *object_table)
{
	...
	for (i = 0; i < ID_Info_for_dp116[6]; i++)
	{
...
static int mxt_read_info_block(struct mxt_data *mxtdata)
{
	...
	//printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	if(!mxt_read_blks(0, size, ID_Info_for_dp116)){
	//	printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
		return FALSE;
	}
	/* Resize buffer to give space for the object table and checksum */
	num_objects = ID_Info_for_dp116[6];

	size += (num_objects * sizeof(struct mxt_object)) + MXT_INFO_CHECKSUM_SIZE;
	/* Read rest of info block */
	//printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	if(!mxt_read_blks(MXT_OBJECT_START, size - MXT_OBJECT_START, read_buf_global_for_dp116 + MXT_OBJECT_START))
		goto err_free_buf;
	/* Parse object table information */
	//printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	if(!mxt_parse_object_table(mxtdata, read_buf_global_for_dp116 + MXT_OBJECT_START))
		goto err_free_buf;
	//printf("                >>> LWG <<< [%s %d] %s CALL ====== \n", __FILE__, __LINE__, __FUNCTION__);
	return TRUE;
err_free_buf:
...
int mxt_initialize_for_dp116(void)         
{
	struct mxt_data *mxtdata;
...
int initial_check_for_dp116(void)         
{
	if(!INT_CHG_PIN_check_for_dp116())
		printf("FAIL\n");
		return FALSE;
	if(!AVDD_check_for_dp116())
		printf("FAIL\n");
		return FALSE;
	printf("PASS\n");
	return TRUE;
...
void backup_reset_for_dp116(void){
	u8 write_cmd = 0x55;
	usleep(10000);
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+1, 1, &write_cmd);//backup
	usleep(100000);

	write_cmd = 0x01;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+0, 1, &write_cmd);//reset
	//brightsource
	//sleep(1);
	usleep(500000);		// 500m OK, 200ms touch raw data 0000, 300ms 0000 
}

bool atmel_init_i2c_set_slvAddr_depending_channel2_for_dp116(int ch, int slvAddr, unsigned int *result)
{
...
bool release_i2c_set2_for_dp116(void)
{
	FUNC_BEGIN();
...
int INT_CHG_PIN_check_for_dp116(void)
{
	u8 INACT_cmd = 0x02;//HIGH = inactive
	u8 ACT_cmd = 0x03;//LOW = active

	if(!mxt_write_blks_for_dp116(touch_table.t18_cfg_address+1, 1, &INACT_cmd)){
		return FALSE;
	}

	if(!mxt_write_blks_for_dp116(touch_table.t18_cfg_address+1, 1, &ACT_cmd)){
		return FALSE;
	}
...
int AVDD_check_for_dp116(void)
{
	...
	mxt_write_blks_for_dp116(touch_table.t25_cfg_address+1, 1, &write_cmd);
...
int FW_Ver_check_for_dp116(model_dp116_t* dp116_p)
{
	char result_buf[10]={0,};
	char result_buf2[10]={0,};
	
	int len1, len2;
	sprintf(result_buf, "%02X", ID_Info_for_dp116[2]);
	sprintf(result_buf2, "%C.%C.%02X",result_buf[0], result_buf[1], ID_Info_for_dp116[3]);
	...
	if((len1 == len2) && (!strncmp("0.0.04", result_buf2, 6))){		// equal(0), not equal(n)
		dp116_p->touch_ic_kind = SOLOMON;
	}else{
		dp116_p->touch_ic_kind = SYNAPTICS;	// SYNAPTICS touch test function is in synaptics_touch_02.c, not in atmel_touch_dev_03.c
	}
...
int CONFIG_Ver_check_for_dp116(void)
{
	char result_buf[10]={0,};
	u8 p[120] = {0,};
	u8 *config_buf = p;
	...
	u8 backup = 0x55;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+1, 1, &backup);//backup
	
	u8 reportall = 0x01;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+3, 1, &reportall);
...
int atmel_03_product_id_check(void)
{
	u8 backup = 0x81;
	u8 p[120] = {0,};
	u8 *config_buf = p;
	...
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+5, 1, &backup);
...
int pin_fault_check_for_dp116(void)
{
	u8 write_cmd = 0x12;
	u8 p[120] = {0,};
	u8 *read_buf = p;
...
int force_touch_check_for_dp116(void)
{
	u8 write_cmd = 0xF8;
	u8 p[120] = {0,};
	u8 *read_buf = p;
	...
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+5, 1, &write_cmd);//ref selfcap mode
	write_cmd = 0x01;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+5, 1, &write_cmd);//page_up1
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address+5, 1, &write_cmd);//page_up2
...
int atmel_03_Node_detection_check(void)
{
...
//	u8 *read_buf = p;
	...
	u8 write_cmd = 0x22;
	mxt_write_blks_for_dp116(touch_table.t8_cfg_address + 0, 1, &write_cmd);
	usleep(10000);

	write_cmd = 0x0A;
	mxt_write_blks_for_dp116(touch_table.t46_cfg_address + 0, 1, &write_cmd);
	usleep(10000);

	write_cmd = 0x00;
	mxt_write_blks_for_dp116(touch_table.t56_cfg_address + 0, 1, &write_cmd);
	usleep(10000);

	write_cmd = 0x00;
	mxt_write_blks_for_dp116(touch_table.t65_cfg_address + 0, 1, &write_cmd);
	usleep(10000);
	
	backup_reset_for_dp116();

	write_cmd = 0x11;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address + 5, 1, &write_cmd);//ref mode
	usleep(10000);

	mxt_read_page(touch_table.t37_cfg_address, byte_len, read_buf_global_for_dp116);

	usleep(10000);
	int i=0;
	for(i=0; i< (16 * 34 * 2 / 32); i++)
		memmove(read_buf_global_for_dp116 + (i * 32), &(read_buf_global_for_dp116[i * 40]), 32);

	if(!node_data_check_for_dp116(read_buf_global_for_dp116, len, l_limit.nodeDetection_MIN, l_limit.nodeDetection_MAX))
		return FALSE;
	return TRUE;
}

int micro_defect_check_for_dp116(void)
{
	int len = PT_CH_SIZE_X * PT_CH_SIZE_Y * 2;
	int page_len = 11;
	int one_page = (2 + 64 + 64);
...
//	u8 *read_buf = p;
	...
	if(!micro_data_check_for_dp116(read_buf_global_for_dp116, len, (long *)l_limit.micro_defect))
		return FALSE;
	return TRUE;
}

int atmel_03_delta_limit_check(void)
{
	int len = PT_CH_SIZE_X * PT_CH_SIZE_Y * 2 ;
	int page_len = 11;
	int one_page = (2 + 64 + 64);
...
//	u8 *read_buf = p;
	...
	write_cmd = 0x10;
	mxt_write_blks_for_dp116(touch_table.t6_cfg_address + 5, 1, &write_cmd);//delta mode
	...
	if(!delta_data_check_for_dp116(read_buf_global_for_dp116, len, l_limit.delta_MIN, l_limit.delta_MAX))
		return FALSE;
	return TRUE;
}

void config_restore_for_dp116(void){
	u8 write_cmd = 0x22;
	mxt_write_blks_for_dp116(touch_table.t8_cfg_address + 0, 1, &write_cmd);
	usleep(10000);
...
int atmel_03_touch_limit_table_parser(MODEL id, char *m_name, struct atmel_03_touch_limit* limit)
{
	char string[500];
	FILE *fp;
	char *token = NULL;
		...
		while(token != NULL){
			...
			else
			{
				...
				{	
					...
					case 1:
						if(atmel_03_touch_csv_parser2(&x, &y, &row, token, limit->fw_ver))
							mode = 0; 
						break;
					case 2:
						if(atmel_03_touch_csv_parser2(&x, &y, &row, token, limit->config_ver))
							mode = 0; 
						break;
					case 3:
						if(atmel_03_touch_csv_parser2(&x, &y, &row, token, limit->product_id))
							mode = 0; 
						break;
					case 4:
						if(atmel_03_touch_csv_parser(&x, &y, &row, token, limit->nodeDetection_MIN))
							mode = 0; 
						break;
					case 5:
						if(atmel_03_touch_csv_parser(&x, &y, &row, token, limit->nodeDetection_MAX))
							mode = 0; 
						break;
					case 6:
						if(atmel_03_touch_csv_parser(&x, &y, &row, token, limit->delta_MIN))
							mode = 0; 
						break;
					case 7:
						if(atmel_03_touch_csv_parser(&x, &y, &row, token, limit->delta_MAX))
							mode = 0; 
						break;
					case 8:
						if(atmel_03_touch_csv_parser(&x, &y, &row, token, limit->micro_defect))
							mode = 0; 
						break;
					case 9:
						if(atmel_03_touch_csv_parser2(&x, &y, &row, token, limit->force_touch_MIN))
							mode = 0; 
						break;
					case 10:
						if(atmel_03_touch_csv_parser2(&x, &y, &row, token, limit->force_touch_MAX))
							mode = 0; 
						break;
				}	
			}
...
int atmel_03_touch_csv_parser(int *xx, int *yy, int *row, char *token, long limit_array[][300])
{
	int i = 0, j =0;
	int n = 0;
	int flag = 0;
...
int atmel_03_touch_csv_parser2(int *xx, int *yy, int *row, char *token, char *limit_array)
{
...
void atmel_03_init_limit_data(struct atmel_03_touch_limit *limit)
{
	l_limit = *limit;
}

int atmel_03_TRANS_RESULT(unsigned char *buf)
{
	int result=0;
...
signed short atmel_03_TRANS_RESULT2(unsigned char *buf)
{
...
int node_compare_result_for_dp116(long MIN[300][300],long MAX[300][300], int col, int row, unsigned char* buf, int size)
{
	...
	for(i=0; i<row; i++)
	{
		for(j=0; j<col; j++)
		{
			...
			int i2c_result = atmel_03_TRANS_RESULT(trans_array);
...
int delta_compare_result_for_dp116(long MIN[300][300],long MAX[300][300], int col, int row, unsigned char* buf, int size)
{
	int i,j,size_count = 0;
	...
	for(i=0; i<row; i++)
	{
		for(j=0; j<col; j++)
		{
			...
			trans_array[0]=(unsigned char) *(buf+size_count);
			trans_array[1]=(unsigned char) *(buf+size_count+1);
			signed short i2c_result = atmel_03_TRANS_RESULT2(trans_array);
...
int micro_compare_result_for_dp116(long MARGIN[300][300], int col, int row, unsigned char* buf, int size)
{
	int i,j,size_count = 0;
	...
	for(i=0; i<row; i++)
	{
		for(j=0; j<col; j++)
		{
			...
			trans_array[0]=(unsigned char) *(buf+size_count);
			trans_array[1]=(unsigned char) *(buf+size_count+1);
			int present_result = atmel_03_TRANS_RESULT(trans_array);
			int next_result = 0;
			if(size_count >= size-2 || j == (col-1)){
				trans_array[0]=(unsigned char) *(buf+size_count-2);
				trans_array[1]=(unsigned char) *(buf+size_count-1);
				next_result = atmel_03_TRANS_RESULT(trans_array);
			}else{
				trans_array[0]=(unsigned char) *(buf+size_count+2);
...
int node_data_check_for_dp116(unsigned char* buf, int size, long MODE0_MIN[300][300], long MODE0_MAX[300][300])
{
	if(node_compare_result_for_dp116(MODE0_MIN, MODE0_MAX, PT_CH_SIZE_X, PT_CH_SIZE_Y, buf, size)){
		return TRUE;
	}else{
		return FALSE;
	}
...
int delta_data_check_for_dp116(unsigned char* buf, int size, long MODE0_MIN[300][300], long MODE0_MAX[300][300])
{
	if(delta_compare_result_for_dp116(MODE0_MIN, MODE0_MAX, PT_CH_SIZE_X, PT_CH_SIZE_Y, buf, size)){
		return TRUE;
	}else{
		return FALSE;
	}
...
int micro_data_check_for_dp116(unsigned char* buf, int size, long MODE0_MIN[300][300])//case margin % ,ex = Micro_defect
{
	if(micro_compare_result_for_dp116(MODE0_MIN, PT_CH_SIZE_X, PT_CH_SIZE_Y, buf, size))
		return TRUE;
	else
		return FALSE;
	return TRUE;
